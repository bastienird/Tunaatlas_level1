...
if(any(data$unit == "MT")) data[data$unit == "MT", ]$unit <- "t"
		if(any(data$unit == "NO")) data[data$unit == "NO", ]$unit <- "no"
		if(any(data$unit == "MTNO")) data[data$unit == "MTNO", ]$unit <- "t"
		if(any(data$unit == "NOMT")) data[data$unit == "NOMT", ]$unit <- "no"
...
# summary(test3)
tm_shape(test3 %>% mutate(st_area = as.factor(st_area.x.x)) %>% filter(continent == "continent"))+tm_polygons(col = "value")+tm_facets("st_area")
...
mislocated <- inner_join %>% ungroup() %>% select(area_competence, source_authority, value, unit, geom) %>% mutate(area_competence = toupper(area_competence)) %>%  mutate(mislocated = case_when(area_competence%in%c("IATTC", "ICCAT", "IOTC", "WCPFC") & area_competence == source_authority ~ TRUE,
...
```{r}
tm_shape(st_as_sf(mislocated %>% filter(mislocated ==FALSE)))+tm_polygons(col = "source_authority")+tm_shape(IOTC)+tm_fill(alpha = 0.2, col = "red")+tm_shape(IATTC)+tm_fill(alpha = 0.2, col = "yellow")+tm_shape(ICCAT)+tm_fill(alpha = 0.2, col = "purple")+tm_shape(WCPFC)+tm_fill(alpha = 0.2, col = "skyblue")
```
