...
date: '2022-03-14'
params:
  chemin_dacces: "~/Documents/Tunaatlas_level1/jobs/difference_L1_L2/entities/global_catch_5deg_1m_IRD_level2/Markdown/endlevel0data/rds.rds"
  plotting: "view"
---
...
# df <- st_as_sf(inner_join(world_sf %>% as.data.frame() %>% select(-geom), all_the_data ,by = "code"))
...
# all_the_data3 <- inner_join %>% mutate(continent = ifelse(area_competence.x != "continent", "ocean", "continent"))
# # summary(all_the_data3)
# tm_shape(all_the_data3 %>% mutate(st_area = as.factor(st_area.x.x)) %>% filter(continent == "continent"))+tm_polygons(col = "value")+tm_facets("st_area")+tm_shape(continent)+tm_borders()
}
...
```{r}
tm_shape(WCPFC_shape)+tm_borders( col = "red")+tm_shape(ICCAT_shape)+tm_borders( col = "yellow")+tm_shape(IOTC_shape)+tm_borders( col = "purple")+tm_shape(IATTC_shape)+tm_borders( col = "skyblue")+tm_shape(st_as_sf(mislocated %>% filter(mislocated ==FALSE) %>% filter(source_authority!="ccsbt")))+tm_polygons(col = "source_authority")+tm_text("area_competence")+tm_facets("unit")
```
...
table <- recap_multpile_unique_SA%>% mutate(pourcentage_value = prop.table(value)*100, pourcentage_row = prop.table(nrow)*100)%>%
  ...
  mutate(ypos_ligne_value = cumsum(pourcentage_value)- 0.5*pourcentage_value) %>% group_by(unit) %>%arrange(unit,desc(pourcentage_row)) %>% mutate(ypos_ligne_row =  cumsum(pourcentage_row)- 0.5*pourcentage_row ) %>%
    distinct()
```
...
data_where_overlap_with_all_informations <- data_where_overlap_with_all_informations%>% inner_join(species_group, by = c("species"))%>% mutate(cat_geo = as.factor(case_when(st_area == 1 ~ "1 deg", st_area == 25 ~ "5 deg", TRUE ~ "> 5 deg")))
...
```{r}
ggplot(data_where_overlap_with_all_informations %>% group_by(unit,gear, source_authority, area_competence) %>% summarise(value = sum(value, na.rm = TRUE)) %>% filter(source_authority != "CCSBT")) +
  aes(
    x = gear,
    colour = source_authority,
    weight = value
