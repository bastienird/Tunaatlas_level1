---
title: 'Analyse of a dataset at a given step of treatment in GlobalTuna Atlas'
author: "IRD"
...
```{r results='asis'}
cat2(paste0("## Temporal coverage of : ", parameter_fact))
```
...
```{r, eval=!is_there_no, fig.cap=paste0("Cumulated catch evolution in tons ", `titre_1`, "dataset ") }
...
```{r echo=F, eval = is_there_no,  out.width = "50%",fig.show='hold',fig.cap="Cumulated catch evolution in tons and number of fish"}

par(mfrow=c(1,2))
ggplot_capt_par_time("Number of fish", legend = FALSE)
ggplot_capt_par_time("Tons", legend = FALSE)
...
```{r results='asis'}
cat2(paste0("## Geographical coverage of : ", parameter_fact))
```
...
if ("Gear"%in% colnames(init) & "Gear"%in% colnames(init) ){
  list_pie_chart <- list("fishingfleet","cat_geo", "source_authority","Gear" #,"species", "species_group"
  )} else {list_pie_chart <- list("fishingfleet","cat_geo", "source_authority", "Name_En"# ,"species", "species_group"
                                                         )}
```
...
cat2(paste0("## Taxonomic coverage of : " ,parameter_fact))

t <- resume_knit_child(list("species_group", "species"))
...
```{r fig.cap="Repartition of the taxonomic coverage", out.width = "50%",fig.show='hold'  }
par(mfrow=c(1,2))
studycatchesird::pie_chart_2(species, init, titre_1 = "", title_yes_no = FALSE)
studycatchesird::pie_chart_2(species_group, init, titre_1 = "", title_yes_no = FALSE)
```
...
data <- dplyr::left_join(data, irregular_iotc %>% st_set_geometry(NULL), by =c("geographic_identifier"= "code")) %>% dplyr::mutate(geographic_identifier=dplyr::coalesce(code_cwp, geographic_identifier)) #%>% dplyr::select(-c(code_cwp, st_area))
...
```{r}
mislocation <- base::ifelse(nrow(mislocated_continent)==0, FALSE, TRUE)
...
data <- dplyr::left_join(init, irregular_iotc %>% st_set_geometry(NULL), by =c("geographic_identifier"= "code")) %>% dplyr::mutate(geographic_identifier=dplyr::coalesce(code_cwp, geographic_identifier)) %>% dplyr::select(-c(code_cwp, st_area))
