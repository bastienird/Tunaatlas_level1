...
knitr::opts_chunk$set(
	echo = TRUE,
	message = debugging,
	warning = TRUE, fig.align = 'center', error = 0
)
...
parameters_child <- list(config = config, entity = entity,opts = opts ,action = action, parameter_filtering = parameter_filtering, parameter_fact = parameter_fact, parameter_con = con,parameter_host =config$software$input$dbi_config$parameters$host, parameter_port = config$software$input$dbi_config$parameters$port,  parameter_user = config$software$input$dbi_config$parameters$user,parameter_dbname=config$software$input$dbi_config$parameters$dbname, parameter_password = config$software$input$dbi_config$parameters$password,  child = TRUE, no_recap_options = TRUE, plotting_type = "plot",parameter_colnames_to_keep = c("fishingfleet",         "gear",                 "time_start",                 
                      "geographic_identifier","schooltype",           "species",                       
                      "unit",                 "value",                "source_authority"), treatment = FALSE)
...
df <- data.frame(matrix(ncol =13, nrow = 1))
      colnames(df) <- c(paste0(tail(str_split(paste0(sub_list_dir_2),"/")[[1]],n=1)),
          ...
          tons_init <- sum((main %>% dplyr::filter(unit%in%c("MTNO", "MT","t")))$value)
          nofish_init <- sum((main %>% dplyr::filter(unit%in%c("NOMT", "NO","no")))$value)
      ...
      for (i in sub_list_dir_2){
        ...
        if (file.exists(paste0(i,"/options_written.txt"))){
          ...
          sum_t <- sum((main %>% dplyr::filter(unit%in%c("MTNO", "MT","t")))$value)
          sum_no <- sum((main %>% dplyr::filter(unit%in%c("NOMT", "NO","no")))$value)
          nrow <- nrow(main)
