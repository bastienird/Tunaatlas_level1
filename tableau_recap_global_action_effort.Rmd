...
parameters_child <- list(config = params$config, entity = params$entity,opts = opts ,action = action, parameter_filtering = parameter_filtering, parameter_fact = action$options$fact, parameter_con = con, child = TRUE)
child_env = new.env()
child_env <- parameters_child
# }
```
...
res2 <- knitr::knit_child("comp_sans_shiny_child_effort.Rmd", envir = child_env, quiet = TRUE)
```
...
res <- knitr::knit_child("comp_sans_shiny_child_effort.Rmd", envir = child_env, quiet = TRUE)
```
...
if(readRDS(paste0(as.character(child_env$parameter_final),"/rds.rds")) == readRDS(paste0(as.character(child_env$parameter_init),"/rds.rds"))){NA}else{
