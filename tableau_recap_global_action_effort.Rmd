...
parameters_child <- list(config = params$config, entity = params$entity,opts = opts ,action = action, parameter_filtering = parameter_filtering, parameter_fact = action$options$fact, parameter_con = con,parameter_host =config$software$input$dbi_config$parameters$host, 
                                                     ...
                                                     parameter_password = config$software$input$dbi_config$parameters$password,  child = TRUE)
child_env = new.env()
...
qflextable2 =function(x, captionn = NULL, autonumm = autonum, pgwidth = 6, colour = FALSE, coloured_columns = NULL){

  y <- x %>% dplyr::mutate_if(is.factor, as.character) %>% dplyr::mutate_if(is.character, ~str_replace_all( ., ",", ", \n" )) %>% 
  ...
  flextable_captionned <- flextable_captionned%>%
...
df <- data.frame(matrix(ncol =13, nrow = 1))
      colnames(df) <- c(paste0(tail(str_split(paste0(sub_list_dir_2),"/")[[1]],n=1)),
                        ...
                        "Options", "Sum in tons", "Sum in number of fish", "Number of lines","Difference (in % of tons)","Difference in tons","Difference (in % of fish)", "Difference in number of fish", "Difference (in % of lines)", "Percentage of nominal"
      )
...
df <- data.frame(matrix(ncol =13, nrow = 1))
      colnames(df) <- c(paste0(tail(str_split(paste0(sub_list_dir_2),"/")[[1]],n=1)),
                        "Explenation", "Fonctions", 
                        "Options", "Sum in hooks", "Sum in fishing days", "Number of lines","Difference (in % of hooks)","Difference in hooks","Difference (in % of fishing days)","Difference in fishing days",  "Difference (in % of lines)", "Percentage of nominal"
      )
