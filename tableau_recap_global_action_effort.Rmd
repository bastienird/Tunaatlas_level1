...
parameter_filtering <- if(!is.null(opts$filtering)){opts$filtering} else{ list(species = NULL, fishingfleet = NULL)}
# if(exists('params')){parameters <- params}else{
parameters <- list(config = config, entity = entity,opts = opts ,action = action, parameter_filtering = parameter_filtering, parameter_fact = parameter_fact, parameter_con = con, child = TRUE)
# }
```
...
assign("titre_1", entity$identifiers[["id"]], envir = environment())
  ...
  assign("parameter_fact", parameter_fact, envir = environment())
