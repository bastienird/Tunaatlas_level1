...
function_all_comp <- function(counting){
  ...
  res <- knitr::knit_child("comp_sans_shiny_child_effort.Rmd", envir = globalenv(), options = do.call(list, unlist(list(parameters, parameter_init = sub_list_dir_2[counting], parameter_final = sub_list_dir_2[counting+1],  step_title = paste0(" Treatment : " ,last_path(parameter_final)), parameter_con=parameter_con,parameter_fact = parameter_fact ), recursive=FALSE)), quiet = TRUE)
}
