...
assign("titre_1", params$entity$identifiers[["id"]], pos = environment())
...
function_all_comp <- function(counting){
  ...
  res <- knitr::knit_child("comp_sans_shiny_child_effort.Rmd", pos = environment(), options = do.call(list, unlist(list(parameters, parameter_init = sub_list_dir_2[counting], parameter_final = sub_list_dir_2[counting+1],  step_title =step_title_treatment, parameter_short = TRUE, child_header = "## "  ), recursive=FALSE)), quiet = TRUE)
}
