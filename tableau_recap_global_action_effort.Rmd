...
opts <- params$action$options
con <- params$config$software$output$dbi
parameter_fact = params$action$options$fact
parameter_fact = action$options$fact
if(!is.null(params$action$options$filtering)){parameter_filtering <-params$action$options$filtering} else{ parameter_filtering <-list(species = NULL, fishingfleet = NULL)}
# if(exists('params')){parameters <- params}else{
parameters_child <- list(config = params$config, entity = params$entity,opts = opts ,action = action, parameter_filtering = parameter_filtering, parameter_fact = action$options$fact, parameter_con = con,parameter_host =config$software$input$dbi_config$parameters$host, 
                                                                           parameter_port = config$software$input$dbi_config$parameters$port, 
                                                                           parameter_user = config$software$input$dbi_config$parameters$user,
                                                                           parameter_dbname=config$software$input$dbi_config$parameters$dbname,
                                                     parameter_password = config$software$input$dbi_config$parameters$password,  child = TRUE, no_recap_options = TRUE)
child_env = new.env()
...
qflextable2 =function(x, captionn = NULL, autonumm = autonum, pgwidth = 4, colour = FALSE, coloured_columns = NULL){
  ...
  dplyr::mutate_if(is.character,~str_replace_all( ., "_", "-" ))  %>%  dplyr::mutate_if(is.character,~str_replace_all( ., "\n", " " )) %>%  dplyr::mutate_if(is.character,~str_replace_all( ., "  ", " " ))%>%mutate_if(is.character, toString)%>% dplyr::mutate_if(is.numeric, function(.){round(.,2)})
...
if(!exists("parameter_fact")){browser()}

  if(exists("parameter_fact")){
...
child_env <- new.env()
list2env(parameters_child, env = child_env)
child_env$titre_1 <- params$entity$identifiers[["id"]]
child_env$parameter_init <- sub_list_dir_2[1]
child_env$parameter_final <- NULL
child_env$parameter_fact <- params$action$options$fact
child_env$child <- TRUE
child_env$no_recap_options <- FALSE
...
child_env$parameter_con <- con
  ...
  # assign("parameter_fact", params$action$options$fact, pos = environment())
...
child_env$title_1 <- "Initial_data"
child_env$title_2 <- "Final_data"
child_env$last_to_first <- TRUE
child_env$parameter_mapped <- TRUE
...
df <- data.frame(matrix(ncol =13, nrow = 1))
      ...
      for (i in sub_list_dir_2){
        ...
        data_i <- cbind(step,
                        Explenation, Fonctions, 
                        Options,
                        sums, loss_percent,loss_tons,loss_percent_no,loss_no, loss_percent_lines, percentage_of_nominal)
      ...
      reduced <- df2 %>% mutate(`Millions of tons` = `Tons` / 1000000, `Millions of fish` = `Number of fish` / 1000000)%>% select(Step, `Millions of tons`, `Millions of fish`, `Lines`)%>% dplyr::mutate(Step_number = as.numeric(row_number()))
...
df <- data.frame(matrix(ncol =13, nrow = 1))
      ...
      for (i in sub_list_dir_2){
        ...
        loss_hooks <- (hooks_init - sum_fdays)
        loss_fdays <- (fishing_days_init - sum_hooks)
        loss_percent_lines <- 100*((lines_init - nrow)/lines_init)
        ...
        data_i <- cbind(step,
                        Explenation, Fonctions, 
                        Options,
                        sums, loss_percent,loss_hooks,loss_percent_fdays,loss_fdays, loss_percent_lines, percentage_of_nominal)
        names(data_i) <- colnames(df)
        df <- rbind(df, data_i)
      ...
      reduced <- df2 %>% mutate(`Sum in millions of hooks` = `Sum in hooks` / 1000000, `Sum in millions of days` = `Sum in fishing days` / 1000000)%>% select(Step, `Sum in millions of hooks`, `Sum in millions of days`, `Lines`)%>% dplyr::mutate(Step_number = as.numeric(row_number()))
      reduced$Step <- factor(reduced$Step, levels = (reduced %>% arrange(Step_number))$Step)
...
qflextable2(reduced, colour = FALSE, captionn ="Evolution of Tons and Number of fish and lines along the steps")
```
...
child_env$params <- params
child_env$titre_1 <- params$entity$identifiers[["id"]]
child_env$parameter_init <- sub_list_dir_2[counting]
child_env$parameter_fact <- params$action$options$fact
child_env$child <- TRUE
child_env$parameter_con <- con
child_env$parameter_final <- sub_list_dir_2[counting+1]
...
child_env$title_2 <- "Final_data"
child_env$last_to_first <- TRUE
child_env$parameter_short <- TRUE
child_env$child_header <- "##"
child_env$step <- counting
child_env$step_title <- paste0(" Treatment : " ,last_path(sub_list_dir_2[counting+1]))
...
ster <- i %>% group_by(Dimension, `New code`) %>% summarise_at(vars(-group_cols()),.funs = function(x){toString(x[!duplicated(x)])}) %>% mutate_all(.funs = function(x){gsub(",$", "",gsub(c("NA", ",NA", " NA"), "", gsub("  ","",gsub("NA,", "", x))))})
...
```{r echo=FALSE, results='asis'}
qflextable2(t, captionn = "Replacement code during the mapping codelist treatment")
```
