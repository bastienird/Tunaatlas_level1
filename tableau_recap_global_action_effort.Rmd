...
parameters <- list(config = params$config, entity = params$entity,opts = opts ,action = action, parameter_filtering = parameter_filtering, parameter_fact = action$options$fact, parameter_con = con, child = TRUE)
# }
```
...
assign("titre_1", entity$identifiers[["id"]], envir = environment())

  assign("parameter_init", sub_list_dir_2[1], envir = environment())
  assign("parameter_fact", action$options$parameter_fact, envir = environment())
  assign("child", TRUE, envir = environment())
  assign("parameter_con", con, envir = environment())
options_knit <- do.call(list, unlist(list(parameters, parameter_init = sub_list_dir_2[1], step_title = "", parameter_fact = parameter_fact), recursive=FALSE))
res2 <- knitr::knit_child("comp_sans_shiny_child_effort.Rmd", envir = environment(), options = options_knit, quiet = TRUE)
```
...
```{r echo=FALSE, results='asis'}
options_knit <-  do.call(list, unlist(list(parameters, parameter_init =sub_list_dir_2[1], parameter_final = sub_list_dir_2[length(sub_list_dir_2)] , parameter_mapped = TRUE, step_title = "", last_to_first = TRUE ), recursive=FALSE))
  assign("parameter_init", sub_list_dir_2[1], envir = environment())
  assign("parameter_final", sub_list_dir_2[length(sub_list_dir_2)] , envir = environment())
  assign("child", TRUE, envir = environment())
  assign("parameter_mapped", TRUE, envir = environment())
  assign("title_1", "Initial_data", envir = environment())
  assign("title_2", "Final_data", envir = environment())
  if(exists("step_title")) {rm(step_title)}

  assign("last_to_first", TRUE, envir = environment())
  assign("parameter_con", con, envir = environment())

res <- knitr::knit_child("comp_sans_shiny_child_effort.Rmd", envir = environment(), options =options_knit, quiet = TRUE)
rm(last_to_first, envir = environment())
rm(title_1, envir = environment())
rm(title_2, envir = environment())
```
...
function_all_comp <- function(counting){
    assign("parameter_init", sub_list_dir_2[counting], envir = environment())
  assign("parameter_final", sub_list_dir_2[counting+1], envir = environment())
  assign("parameter_short", TRUE, envir = environment())
  assign("child", TRUE, envir = environment())
  assign("step", counting, envir = environment())
  assign("child_header", "##", envir= environment())
  assign("step_title", paste0(" Treatment : " ,last_path(parameter_final)), envir= environment())
  rm(parameter_mapped)

  step_title_treatment <-  paste0(" Treatment : " ,last_path(sub_list_dir_2[counting+1]))
  res <- knitr::knit_child("comp_sans_shiny_child_effort.Rmd", envir = environment(), options = do.call(list, unlist(list(parameters, parameter_init = sub_list_dir_2[counting], parameter_final = sub_list_dir_2[counting+1],  step_title =step_title_treatment, parameter_short = TRUE, child_header = "## "  ), recursive=FALSE)), quiet = TRUE)
}
