...
if (isNullList(params$filtering)){cat("There are no filter on this data")} else {
  
  cat (paste0("The filter used on this data are:  \n "))
  for (i in 1:length(params$filtering)){
    if (!is.null(params$filtering[[i]])){
      cat(paste0("***- On ", names((params$filtering)[i]), " : ", paste((params$filtering)[[i]], collapse = " ; "),"*** \n"))
    } else {""}
  }
}
...
if(dir.exists("Markdown")){
  ...
  if(params$fact == "catch"){
    nominal_dataset <- readr::read_csv(paste0(getwd(),"/data/global_nominal_catch_firms_level0.csv"))}else if (params$fact == "effort"){
...
matchingList <- params$filtering %>% purrr::keep( ~ !is.null(.) )
...
res2 <- knitr::knit_child("comp_sans_shiny_child_effort.Rmd", envir = environment(), options = list(params, step_title = "", parameter_init = parameter_init, child = TRUE, parameter_con = parameter_con), quiet = TRUE)
```
...
assign("child", TRUE, envir = globalenv())
  ...
  assign("parameter_final", sub_list_dir_2[length(sub_list_dir_2)], envir = globalenv())
    assign("step_title", "", envir= globalenv())
    assign("child_header", "#", envir= globalenv())
  

res <- knitr::knit_child("comp_sans_shiny_child_effort.Rmd", envir = globalenv(), options = params, quiet = TRUE)
...
```{r include=FALSE, eval=(params$fact == "catch")}
...
```{r include=FALSE , eval=(params$fact == "effort")}
if(!exists("nominal")){nominal <- 1}
df <- data.frame(matrix(ncol =12, nrow = 1))
      colnames(df) <- c(paste0(tail(str_split(paste0(sub_list_dir_2),"/")[[1]],n=1)),
                        "Explenation", "Fonctions", 
...
```{r echo=FALSE, fig.cap='Evolution of captures in fishing days and hooks during the process',results='asis', eval=(params$fact == "effort")}
...
function_all_comp <- function(counting){
  ...
  res <- knitr::knit_child("comp_sans_shiny_child_effort.Rmd", envir = globalenv(), options = list(params, parameter_init = sub_list_dir_2[counting], parameter_final = sub_list_dir_2[counting+1], child_header = "#", step_title = paste0("Treatment : " ,last_path(parameter_final)), parameter_con=parameter_con), quiet = TRUE)
}
