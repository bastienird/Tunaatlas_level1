...
- ***`r params$final` for the initial data***
...
init <- fonction_filtre(readRDS(paste0(as.character(params$final), "/rds.rds")) %>% dplyr::select(all_of(colnames_to_keep))  )

  if (is.null(params$titre_dataset_1)){
    titre_1 <- last_path_reduced(as.character(params$final))
  } else {
...
fonction_groupement = function(x, init){
  x  <-   enquo(x)
  groupement_1  <-   init %>% group_by(!!x ) %>% dplyr::summarise(value_sum_1 = sum(value, na.rm=TRUE), number_lines1 = n())
}
```
...
```{r}
captures_par_time <- testtime_start %>% mutate(Time = as.Date(time_start))%>% dplyr::rename(`Captures table 1` = "value_sum_1")%>% pivot_longer(cols = c(`Captures table 1`), names_to = "Origin", values_to ="Captures")
...
```{r}
captures_par_time <- captures_par_time %>% mutate(Time = as.Date(time_start))%>% arrange(time_start) %>% group_by(Origin) %>%  mutate(`Captures cumulées` = cumsum(`Captures`)) %>% dplyr::distinct()
...
```{r}
rawdata<- init #%>% dplyr::filter(source_authority == params$source_authority)
rdslevel0 <- rawdata
...
conversion_factor_level0 <- rbind(dplyr::inner_join(strata_nomt , strata_mtno, by = setdiff(colnames(strata_mtno), c("value", "unit") )) %>% dplyr::rename(NO =value.x, MT = value.y) %>%
                                    dplyr::group_by_at(vars(one_of(colnames_to_keep))) %>% 
...
try(studycatchesird::pie_chart_2(fishingfleet,mislocated_continent_init,rawdata))
