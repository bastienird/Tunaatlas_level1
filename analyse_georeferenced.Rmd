...
init <- fonction_filtre(readRDS(paste0(as.character(params$final), "/rds.rds")) %>% select(all_of(colnames_to_keep)) %>%  left_join(species_group, by = c("species"))%>% distinct() )
...
```{r}
# studycatchesird::pie_chart_2 = function(x, init= init) {
