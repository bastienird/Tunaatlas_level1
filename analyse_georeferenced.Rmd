...
conversion_factor_level0 <- rbind(dplyr::inner_join(strata_nomt , strata_mtno, by = setdiff(colnames(strata_mtno), c("value", "unit") )) %>% dplyr::rename(NO =value.x, MT = value.y) %>%
                                    dplyr::group_by(across(all_of(colnames_to_keep))) %>% 
...
strange <- pie_chart_2(fishingfleet,first=absurd_conversion_factor_tons,second=rawdata )
strange
```
...
pie_chart_2(fishingfleet,mislocated_continent_init,rawdata)
