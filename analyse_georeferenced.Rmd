...
```{r}
rdslevel0 <- readRDS(paste0(as.character(params$final), "/rds.rds"))
...
out <- try((rawdata <- readRDS("Markdown/mapping_codelist/rds.rds")), silent = TRUE)
if(!any(class(out) == "error")){rawdata <- readRDS("Markdown/rawdata/rds.rds")}
...
try(pie_chart_2(fishingfleet,mislocated_continent_init,rawdata))
...
# 
# plot_grid(mislocated, original)
pie_chart_2(species,mislocated_continent_init,rawdata)
