...
if (nrow(strates_perdues_first_10)!=0){
  groupe_df <- as_grouped_data(strates_perdues_first_10%>% mutate(`Loss / Gain` = ifelse(loss >= 0, "Loss", "Gain"))  %>%  mutate(`Loss / Gain` = case_when(is.na(`Loss / Gain`) ~ "Gain",value_sum_1 == value_sum_2~"Egal", TRUE ~ `Loss / Gain`))%>% ungroup()%>% rename(`Captures table 1` = "value_sum_1",`Captures table 2` = "value_sum_2") %>% group_by(Dimension, unit,`Loss / Gain`) %>% arrange(Dimension,unit,`Loss / Gain`)%>% select(params$columns_to_keep)%>% mutate_if(is.numeric, round, digits=2), groups = c("Dimension","unit","Loss / Gain"))
...
```{r paged.print=TRUE}
t3 <- as_grouped_data(unit %>% ungroup()%>% mutate(`Loss / Gain` = ifelse(loss >= 0, "Loss", "Gain")) %>%  mutate(`Loss / Gain` = case_when(is.na(`Loss / Gain`) ~ "Gain",value_sum_1 == value_sum_2~"Egal", TRUE ~ `Loss / Gain`))%>% rename(`Captures table 1` = "value_sum_1",`Captures table 2` = "value_sum_2")%>% select(params$columns_to_keep)%>% mutate_if(is.numeric, round, digits=2), groups = c("Dimension","unit"))
qflextable2(t3) 
```
...
t4 <- head(strates_perdues%>% mutate(`Loss / Gain` = ifelse(loss >= 0, "Loss", "Gain")) %>%  mutate(`Loss / Gain` = case_when(is.na(`Loss / Gain`) ~ "Gain",value_sum_1 == value_sum_2~"Egal", TRUE ~ `Loss / Gain`))%>% rename(`Captures table 1` = "value_sum_1",`Captures table 2` = "value_sum_2")%>% select(params$columns_to_keep)%>% mutate_if(is.numeric, round, digits=2),10)
...
```{r paged.print=TRUE}
t5 <- as_grouped_data(unit %>% ungroup()%>% mutate(`Loss / Gain` = ifelse(loss >= 0, "Loss", "Gain")) %>%  mutate(`Loss / Gain` = case_when(is.na(`Loss / Gain`) ~ "Gain",value_sum_1 == value_sum_2~"Egal", TRUE ~ `Loss / Gain`))%>% rename(`Captures table 1` = "value_sum_1",`Captures table 2` = "value_sum_2")%>% select(params$columns_to_keep)%>% mutate_if(is.numeric, round, digits=2), groups = c("Dimension","unit"))
qflextable2(t5) 
```
