...
params: 
  init: "~/Documents/Tunaatlas_level1/jobs/all_25_avril/entities/global_catch_firms_Bastien_rds_level0/Markdown/level1realocate_remove"
  final: "~/Documents/Tunaatlas_level1/jobs/all_25_avril/entities/global_catch_firms_Bastien_rds_level0/Markdown/rawdata"
...
source_authority <- pie_chart_2(source_authority, init, final, topn = 6)
...
```{r paged.print=TRUE}
t3 <- as_grouped_data(unit %>% dplyr::ungroup()%>%  dplyr::rename(`Captures table 1` = "value_sum_1",`Captures table 2` = "value_sum_2")%>% dplyr::filter(`Loss / Gain` != "Egal")%>%dplyr::select(params$columns_to_keep)%>%dplyr::mutate_if(is.numeric, round, digits=2) %>% group_by(`Loss / Gain`, Dimension, unit) %>% arrange(`Loss / Gain`, Dimension, unit,desc(`Difference in millions of tons`)),groups = c("Loss / Gain","Dimension","unit"))
