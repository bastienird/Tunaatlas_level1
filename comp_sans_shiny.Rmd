...
params: 
  ...
  titre_dataset_1: "NULL"
  titre_dataset_2: "NULL"
...
init <- fonction_filtre(readRDS(as.character(params$init)) %>% select(colnames_to_keep)%>% inner_join(shape_without_geom, by = c("geographic_identifier"="code")))%>% inner_join(species_group, by = c("species"))
...
final <- fonction_filtre(readRDS(as.character(params$final))%>% select(colnames_to_keep)%>% inner_join(shape_without_geom, by = c("geographic_identifier"="code"))) %>% inner_join(species_group, by = c("species"))
...
```{r}
captures_par_time <- testtime_start %>% mutate(Time = as.Date(Precision))%>% rename(`Captures table 1` = "value_sum_1",`Captures table 2` = "value_sum_2")%>% pivot_longer(cols = c(`Captures table 1`, `Captures table 2`), names_to = "origin", values_to ="Captures") 
...
ggplot_capt_par_time = function(x){
  ...
  labs(x = "Time", y = "Cumulated captures (in tons)")+guides(fill=guide_legend(title="Etape du traitement de la donnée"))
}
...
#     mutate(class= as.factor(ifelse(id<4,!!colnames,"Autres"))) %>%
#   # filter(id < 4) %>% 
#   #   mutate(class= !!colnames) %>% 
...
#   labs(x = "", y="")+
#   labs(title = paste0("Distribution in value for the dimension : ",r)) + facet_wrap("unit")
# 
# }
...
```{r eval=FALSE, include=FALSE}
plot_grid(a, h, nrow = 2,labels = c(paste0('table', last_path_reduced(as.character(params$init))), paste0('table', last_path_reduced(as.character(params$final)))),
  label_size = 12,
  label_x = 0, label_y = 0,
  hjust = -0.5, vjust = -0.5
)
plot_grid(b, i, nrow = 2,labels = c(paste0('table', last_path_reduced(as.character(params$init))), paste0('table', last_path_reduced(as.character(params$final)))),
  label_size = 12,
  label_x = 0, label_y = 0,
  hjust = -0.5, vjust = -0.5
)
# plot_grid(c, j, nrow = 2,labels = c(paste0('table', last_path_reduced(as.character(params$init))), paste0('table', last_path_reduced(as.character(params$final)))),
#   label_size = 12,
#   label_x = 0, label_y = 0,
#   hjust = -0.5, vjust = -0.5
# )
plot_grid(d, k, nrow = 2,labels = c(paste0('table', last_path_reduced(as.character(params$init))), paste0('table', last_path_reduced(as.character(params$final)))),
  label_size = 12,
  label_x = 0, label_y = 0,
  hjust = -0.5, vjust = -0.5
)
...
a <- plot_grid(cat_geo, fishing_fleet, nrow=2)
tmap_arrange(cat_geo, fishing_fleet, nrow=2)
```
...
```{r}
pie_chart_2(gear, init, final)
```

```{r}
pie_chart_2(species_group, init, final)
```
...
```{r}
fonction_empreinte_spatiale("t")
```

In numbers 

```{r}
fonction_empreinte_spatiale("no")
```
...
unit_t <- t %>% filter(unit == "t")%>% filter(Dimension %in% c("species", "source_authority","cat_geo", "fishingfleet", "species_group")) %>% mutate(Dimension = as.factor(Dimension)) %>%  group_by( Dimension) %>% arrange(desc(`Loss (in %)`))   %>%   mutate(id = row_number())%>%   mutate(test = as.factor(ifelse(id>(topn-1),paste0("Autres"),paste0(as.character(Precision))))) %>% ungroup() %>% group_by(Dimension,test) %>% summarise(across(is.numeric, sum)) %>% mutate(`Loss (in %)` = (`Perte en millions de tonnes`*1000000/value_sum_1)*100) %>% select(-id)%>%group_by(Dimension) %>%  arrange(Dimension,desc(`Loss (in %)`))

unit_no <- t %>% filter(unit == "no") %>% filter(Dimension %in% c("species", "source_authority","cat_geo", "fishingfleet", "species_group")) %>% mutate(Dimension = as.factor(Dimension)) %>%  group_by( Dimension) %>% arrange(desc(`Loss (in %)`))   %>%   mutate(id = row_number())%>%   mutate(test = as.factor(ifelse(id>(topn-1),paste0("Autres"),paste0(as.character(Precision))))) %>% ungroup() %>% group_by(Dimension,test) %>% summarise(across(is.numeric, sum)) %>% mutate(`Loss (in %)` = (`Perte en millions de tonnes`*1000000/value_sum_1)*100) %>% select(-id)%>%group_by(Dimension) %>%  arrange(Dimension,desc(`Loss (in %)`))



# %>% ungroup()%>% group_by(test,Dimension) %>% mutate_all(sum) %>% ungroup() %>% mutate(`Loss (in %)` =`Perte en millions de tonnes`/sum(`Perte en millions de tonnes`)) %>% mutate(Precision = test) %>%  mutate_if(is.character, as.factor)%>%select(-c(test,id)) %>%  group_by(Dimension) %>%arrange(Dimension,desc(`Loss (in %)`))
# unit_no <- t %>% filter(unit == "no") %>% filter(Dimension %in% c("species", "source_authority","cat_geo", "fishingfleet")) %>% mutate(Dimension = as.factor(Dimension)) %>%  mutate(test = ifelse(`Loss (in %)` >10,"Autres",as.character(Precision))) %>%mutate(Precision = test) %>%    mutate(Perte_pourcent = 100*((value_sum_1 - value_sum_2)/value_sum_1)) %>%  mutate_if(is.character, as.factor)%>%select(-test) %>% ungroup() %>%  group_by(Precision,unit, Dimension)%>% summarise_all(sum)  %>% group_by(unit, Dimension) %>%arrange(desc(`Loss (in %)`), desc(`Perte en millions de tonnes`)) %>% group_by(Dimension)
# library(kable)
```
...
Map presenting for each area, the kept information, the all loss information and the area where only a part of the information is lost.


```{r}
tmap::tmap_mode("view")
tm_shape(inner_join(shapefile.fix,(t %>% filter(Dimension == "geographic_identifier")) , by = c("code"="Precision")) %>% mutate(cat_perte = as.factor(case_when(`Loss (in %)` == 0 ~ 'Pas de perte', `Loss (in %)` == 100~'Perte totale', TRUE ~ 'Perte partielle')))
         # %>% filter(unit == "t")
         )+
