...
params: 
  init: "~/Documents/Tunaatlas_level1/jobs/20220620155133/entities/global_catch_1deg_1m_ps_bb_firms_Bastien_filtering_wcpfc_at_the_end_level0/Markdown/rawdata"
  final: "~/Documents/Tunaatlas_level1/jobs/20220620155133/entities/global_catch_1deg_1m_ps_bb_firms_Bastien_filtering_wcpfc_at_the_end_level0/Markdown/treatment_after_binding"
...
conflict_prefer("group_by_","dplyr")
conflict_prefer("filter","dplyr")
# conflict_prefer_all("dplyr", quiet = TRUE)
# write("options_written", paste0("options.txt"))
...
qflextable2 =function(x){
  if("value_sum_1"%in%colnames(x)){x <- x %>%  dplyr::rename("Tons in table 1" = "value_sum_1")}
  # y <- (x)
  ...
  FitFlextableToPage(qflextable(x))
  }
...
```{r}
species_group <-   read_excel(file.path("data", "SPECIES_LIST_RFMO_WITH_ERRORS.xlsx")) %>% janitor::clean_names() %>%  select(species_group, species_code) %>% dplyr::rename(species = species_code)
...
differences_in_optionsinit <- as.data.frame(stringr::str_split_fixed(differences_in_optionsinit$`Options init`, "=", 2)) %>% dplyr::rename(Options_initial =V2)%>% filter(Options_initial!="")

differences_in_optionsfinal <- as.data.frame(stringr::str_split_fixed(differences_in_optionsfinal$`Options final`, "=", 2)) %>% dplyr::rename(Options_final =V2) %>% filter(Options_final!="")
...
```{r}
s <-  differences_in_optionsinit_base%>% dplyr::rename("Options" = X1, "Value" = X2)
qflextable2(s)
```
...
fonction_groupement = function(x, init, final){
  ...
  fulljoin  <-   full_join(groupement_1, groupement_2)%>% mutate(value_sum_2 = ifelse(is.na(value_sum_2), 0, value_sum_2)) %>%mutate(value_sum_1 = ifelse(is.na(value_sum_1), 0, value_sum_1))%>% mutate(loss = value_sum_1 - value_sum_2) %>% mutate(`Loss / Gain` = ifelse(loss >= 0, "Loss", "Gain")) %>% 
    mutate(Loss_pourcent = 100*((value_sum_1 - value_sum_2)/value_sum_1))%>% mutate(Dimension = colnames(groupement_1[1])) %>%
    dplyr::rename("Precision" = 1) %>% mutate(Precision = as.character(Precision)) %>% mutate(value_sum_2 = ifelse(is.na(value_sum_2), 0, value_sum_2))%>% mutate(Loss_pourcent = ifelse(is.na(Loss_pourcent)|Loss_pourcent==-Inf, 100, Loss_pourcent)) %>% mutate(loss_nb_ligne = number_lines1 - number_lines2)%>% 
    mutate(Loss_en_milliers_de_tonne = (value_sum_1 - value_sum_2)/ 10^6) %>% 
            dplyr::rename(`Difference (in %)` = Loss_pourcent,`Difference in millions of tons` = Loss_en_milliers_de_tonne, 
                  `Difference in number of lines` = loss_nb_ligne) %>% mutate_if(is.numeric, list(~replace_na(., 0))) %>%  mutate(`Loss / Gain` = case_when(is.na(`Loss / Gain`) ~ "Gain",value_sum_1 == value_sum_2~"Egal", TRUE ~ `Loss / Gain`))
}
```
...
strates_perdues <- t %>% filter(value_sum_2 == 0 & `Loss / Gain` == "Loss") 
strates_gagnees <- t %>% filter(value_sum_1 == 0 & `Loss / Gain` == "Gain")
...
strates_perdues_first_10 <- rbind(strates_perdues,strates_gagnees) %>% filter(`Loss / Gain` != 'Egal')%>%ungroup %>% filter(Dimension!="geographic_identifier") %>% group_by(`Loss / Gain`,Dimension)%>% slice(1:10) %>%select(Dimension, everything()) 

if (nrow(strates_perdues_first_10)== 0){strates_perdues_first_10 <- rbind(strates_perdues,strates_gagnees) %>% filter(`Loss / Gain` != 'Egal')%>%ungroup %>% filter(Dimension!="geographic_identifier") %>% group_by(`Loss / Gain`,Dimension)%>% slice(1:10) %>%select(Dimension, everything()) }
...
```{r}
if (nrow(t %>% filter(`Loss / Gain` != 'Egal'))==0){
...
if (nrow(strates_perdues_first_10)!=0){
  groupe_df <- as_grouped_data(strates_perdues_first_10%>% mutate(`Loss / Gain` = ifelse(loss >= 0, "Loss", "Gain"))  %>%  mutate(`Loss / Gain` = case_when(is.na(`Loss / Gain`) ~ "Gain",value_sum_1 == value_sum_2~"Egal", TRUE ~ `Loss / Gain`))%>% ungroup()%>%  dplyr::rename(`Captures table 1` = "value_sum_1",`Captures table 2` = "value_sum_2") %>% group_by(Dimension, unit,`Loss / Gain`) %>% arrange(Dimension,unit,`Loss / Gain`)%>% select(params$columns_to_keep)%>% mutate_if(is.numeric, round, digits=2), groups = c("Dimension","unit","Loss / Gain"))
...
```{r}
captures_par_time <- testtime_start %>% mutate(Time = as.Date(Precision))%>%  dplyr::rename(`Captures table 1` = "value_sum_1",`Captures table 2` = "value_sum_2")%>% pivot_longer(cols = c(`Captures table 1`, `Captures table 2`), names_to = "Origin", values_to ="Captures") %>% mutate(Origin = case_when(Origin == "Captures table 1" ~ titre_1 , TRUE ~ titre_2))
...
```{r paged.print=TRUE}
t3 <- as_grouped_data(unit %>% ungroup()%>% mutate(`Loss / Gain` = ifelse(loss >= 0, "Loss", "Gain")) %>%  mutate(`Loss / Gain` = case_when(is.na(`Loss / Gain`) ~ "Gain",value_sum_1 == value_sum_2~"Egal", TRUE ~ `Loss / Gain`))%>%  dplyr::rename(`Captures table 1` = "value_sum_1",`Captures table 2` = "value_sum_2")%>% select(params$columns_to_keep)%>% mutate_if(is.numeric, round, digits=2), groups = c("Dimension","unit"))
qflextable2(t3) 
```
...
tmap_mode("plot")
inner_join <- inner_join(shapefile.fix,(t %>% filter(Dimension == "geographic_identifier")) , by = c("code"="Precision"))%>% mutate(`Impact on the data` = as.factor(case_when(`Difference (in %)` == 0 ~ 'No differences', `Difference (in %)` == 100~'All data different',`Difference (in %)` < 0 ~ "Gain", TRUE ~ 'Loss')))

if (nrow(inner_join %>% filter(`Impact on the data` == 'All data different'))!=0 & nrow(inner_join %>% filter(`Impact on the data` == 'No differences'))!=0){
image <- tm_shape(inner_join(shapefile.fix,(t %>% filter(Dimension == "geographic_identifier")) , by = c("code"="Precision"))) +
  tm_fill("Difference (in %)", palette="RdYlGn", style="cont", n=8,
				 id="name", midpoint = 0)+ tm_shape(inner_join %>% filter(`Impact on the data` == 'No differences')) + tm_borders(col="skyblue")+tm_shape(inner_join %>% filter(`Impact on the data` == 'All data different')) +tm_borders(col="red")+tm_facets(by = c("unit", "cat_geo"),free.coords = TRUE, free.scales = TRUE)+
...
tmap::tmap_mode("plot")
image <- tm_shape(inner_join(shapefile.fix,(t %>% filter(Dimension == "geographic_identifier")) , by = c("code"="Precision")) %>% mutate(`Impact on the data` = as.factor(case_when(`Difference (in %)` == 0 ~ 'No differences', `Difference (in %)` == 100~'All data lost' ,`Difference (in %)` < 0 ~ "Gain", TRUE ~ 'Loss')))
         # %>% filter(unit == "Tons")
         )+
...
# The species differences (to help for new analysis)
...
The differences is `r loss_en_tonnes` in tons (**`r loss_en_tonnes_pourcent`%**). The differences is `r loss_en_nombre` in number of fish (**`r loss_en_nombre_pourcent`%**).
...
t4 <- head(strates_perdues%>% mutate(`Loss / Gain` = ifelse(loss >= 0, "Loss", "Gain")) %>%  mutate(`Loss / Gain` = case_when(is.na(`Loss / Gain`) ~ "Gain",value_sum_1 == value_sum_2~"Egal", TRUE ~ `Loss / Gain`))%>%  dplyr::rename(`Captures table 1` = "value_sum_1",`Captures table 2` = "value_sum_2")%>% select(params$columns_to_keep)%>% mutate_if(is.numeric, round, digits=2),10)
...
```{r paged.print=TRUE}
t5 <- as_grouped_data(unit %>% ungroup()%>% mutate(`Loss / Gain` = ifelse(loss >= 0, "Loss", "Gain")) %>%  mutate(`Loss / Gain` = case_when(is.na(`Loss / Gain`) ~ "Gain",value_sum_1 == value_sum_2~"Egal", TRUE ~ `Loss / Gain`))%>%  dplyr::rename(`Captures table 1` = "value_sum_1",`Captures table 2` = "value_sum_2")%>% select(params$columns_to_keep)%>% mutate_if(is.numeric, round, digits=2), groups = c("Dimension","unit"))
qflextable2(t5) 
```
...
image <- tm_shape(inner_join(shapefile.fix,(t %>% filter(Dimension == "geographic_identifier")) , by = c("code"="Precision")) %>% mutate(`Impact on the data` = as.factor(case_when(`Difference (in %)` == 0 ~ 'No differences', `Difference (in %)` == 100~'All data different', TRUE ~ 'Partial differences')))
         # %>% filter(unit == "Tons")
         )+
  tm_fill( "value", palette="RdYlGn", style="cont", n=8,
				 id="name", midpoint = 0) +
