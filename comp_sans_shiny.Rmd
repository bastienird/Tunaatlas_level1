...
dataframe_tot_filter <- dataframe_tot_filter %>% left_join(cl_cwp_gear_level2, by = c("gear" = "Code")) %>% rename(Gear = Name_En)
...
The number of lines goes from `r round(nb_ligne_init_millions,3)` millions in table 1 (`r titre_1`) to `r round(nb_ligne_final_millions,3)` millions in table 2 (`r titre_2`), which correspond to a difference of `r round(((nb_ligne_init_millions - nb_ligne_final_millions)/nb_ligne_init_millions)*100,5)`%.
...
cat_geo <- as.list(unique(testcat_geo$Precision))
cat(" \n \n ")
```
...
list_pie_chart <- list("fishingfleet","cat_geo", "source_authority", "species", "species_group","Gear")
