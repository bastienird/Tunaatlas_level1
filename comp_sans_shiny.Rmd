...
params: 
  init: "/home/grasset/Documents/Tunaatlas_level1/jobs/20220525104114/entities/global_catch_1deg_1m_ps_bb_firms_Bastien_with_step_rds__level2/Markdown/Level2_RF2"
  final: "/home/grasset/Documents/Tunaatlas_level1/jobs/20220525104114/entities/global_catch_1deg_1m_ps_bb_firms_Bastien_with_step_rds__level2/Markdown/Level2_RF3without_gears"
...
- ***acces `r params$init` for the initial data***

- ***acces `r params$final` for the final data***
...
```{r}
qflextable(differences_between_options2)
```
...
```{r}
qflextable(differences_in_optionsinit_base)
```
...
qflextable(as_grouped_data(strates_perdues_first_10 %>% ungroup() %>% group_by(Dimension, unit,Loss_gain) %>% arrange(Dimension,unit,Loss_gain), groups = c("Dimension","unit","Loss_gain")))} else {print("No strata is gain nor lost")}
```
...
tm_shape(inner_join %>% filter(unit == variable_affichee))+
  tm_fill("value", palette="RdYlGn", style="cont", n=8,
				 id="name", midpoint = 0) +
  tm_layout(legend.outside = FALSE) + tm_facets(by=c("cat_geo","source"), free.scales = TRUE)
}
}
tmap_mode("plot")
...
```{r}
tmap_mode("view")
inner_join <- inner_join(shapefile.fix,(t %>% filter(Dimension == "geographic_identifier")) , by = c("code"="Precision"))%>% mutate(cat_loss = as.factor(case_when(`Difference (in %)` == 0 ~ 'No differencies', `Difference (in %)` == 100~'All data different', TRUE ~ 'Partial differencies')))

if (nrow(inner_join %>% filter(cat_loss == 'All data different'))!=0 & nrow(inner_join %>% filter(cat_loss == 'No differencies'))!=0){
tm_shape(inner_join(shapefile.fix,(t %>% filter(Dimension == "geographic_identifier")) , by = c("code"="Precision"))) +
  tm_fill("Difference (in %)", palette="RdYlGn", style="cont", n=8,
				 id="name", midpoint = 0)+ tm_shape(inner_join %>% filter(cat_loss == 'No differencies')) + tm_borders(col="skyblue")+tm_shape(inner_join %>% filter(cat_loss == 'All data different')) +tm_borders(col="red")+tm_facets(by = c("unit", "cat_geo"),free.coords = TRUE)+
  tm_layout(legend.outside = FALSE) } else{

tm_shape(inner_join(shapefile.fix,(t %>% filter(Dimension == "geographic_identifier")) , by = c("code"="Precision"))) +
  tm_fill("Difference (in %)", palette="RdYlGn", style="cont", n=8,
				 id="name", midpoint = 0)+ tm_facets(by = c("unit", "cat_geo"),free.coords = TRUE)+
...
tmap::tmap_mode("view")
tm_shape(inner_join(shapefile.fix,(t %>% filter(Dimension == "geographic_identifier")) , by = c("code"="Precision")) %>% mutate(cat_loss = as.factor(case_when(`Difference (in %)` == 0 ~ 'No differencies', `Difference (in %)` == 100~'All data different', TRUE ~ 'Partial differencies')))
         # %>% filter(unit == "Tons")
         )+
  tm_fill( "cat_loss", palette="RdYlGn", style="cont", n=8,
				 id="name", midpoint = 0) +
  tm_layout(legend.outside = FALSE) + tm_facets(by = c("unit", "cat_geo"), free.scales = TRUE)
```
