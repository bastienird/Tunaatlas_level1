...
params: 
  init: "/home/grasset/Documents/Tunaatlas_level1/jobs/20220616115652/entities/global_catch_1deg_1m_ps_bb_firms_Bastien_filtering_wcpfc_at_the_end_level0/Markdown/filtering_on_gear"
  final: "/home/grasset/Documents/Tunaatlas_level1/jobs/20220616115652/entities/global_catch_1deg_1m_ps_bb_firms_Bastien_filtering_wcpfc_at_the_end_level0/Markdown/mapping_codelist"
...
continent <- fSpatPlan_Convert2PacificRobinson(st_read(con, query = query)%>% filter(!st_is_empty(.)))
...
qflextable2 =function(x){
  if("value_sum_1"%in%colnames(x)){x <- x %>% rename("Tons in table 1" = "value_sum_1")}
  # y <- FitFlextableToPage(x)
   save_as_image(qflextable(x), paste0(getwd(),"/",deparse(substitute(x)),".png"))
...
The purpose of this markdown is to describe the differencies of data from the World Tuna Atlas following the different filters. 
...
fonction_filtre = function(dataframe_tot_filter) {
  ...
  dataframe_tot_filter <- dataframe_tot_filter%>% mutate(unit = case_when(unit %in% c("MT","t","MTNO", "Tons")~ "Tons", unit %in% c("NO", "NOMT","no", "Number of fish")~"Number of fish")) %>% mutate(gear = as.character(gear))
...
dataframe_tot_filter_iotc_shape <-dataframe_tot_filter %>% inner_join(irregular_iotc , by =c("geographic_identifier"= "code")) %>%
  ...
  select(-c(code_cwp))%>% group_by(across(setdiff(everything(),"value"))) %>% summarise(value =sum(value))

dataframe_tot_filter <- rbind(dataframe_tot_filter_normal ,dataframe_tot_filter_iotc_shape) %>% ungroup()
}

  
init <- fonction_filtre(readRDS(paste0(as.character(params$init),"/rds.rds")) %>% select(all_of(colnames_to_keep)))%>%  left_join(shape_without_geom, by = c("geographic_identifier"="code")) %>% mutate(time_start =as.character(time_start))%>% mutate(time_end =as.character(time_end)) %>% mutate(cat_geo = as.character(cat_geo))

final <- fonction_filtre(readRDS(paste0(as.character(params$final),"/rds.rds"))%>% select(all_of(colnames_to_keep))) %>% 
                            left_join(shape_without_geom, by = c("geographic_identifier"="code"))  %>% mutate(time_start =as.character(time_start))%>% mutate(time_end =as.character(time_end))%>% mutate(cat_geo = as.character(cat_geo))
...
```{r}
differences_in_optionsinit <- readr::read_delim(paste0(params$init,"/options_total.txt") ,col_names = FALSE,delim="=" ) %>% distinct() 
library(tidyr)
...
differences_in_optionsinit_base <- differences_in_optionsinit %>% distinct()


differences_in_optionsfinal <- readr::read_delim(paste0(params$final,"/options_total.txt"), col_names = FALSE, delim = "=") %>% distinct() 
...
library(stringr)
differences_in_optionsinit <- as.data.frame(stringr::str_split_fixed(differences_in_optionsinit$`Options init`, "=", 2)) %>% rename(Options_initial =V2)%>% filter(Options_initial!="")

differences_in_optionsfinal <- as.data.frame(stringr::str_split_fixed(differences_in_optionsfinal$`Options final`, "=", 2)) %>% rename(Options_final =V2) %>% filter(Options_final!="")

differences_between_options <- full_join(differences_in_optionsfinal,differences_in_optionsinit) %>% distinct()

differences_between_options[is.na(differences_between_options)] = "none"

differences_between_options2 <-differences_between_options %>% filter(Options_final != Options_initial)

```

The differences in options choosen between the two dataframes are : 
...
} else { towrite ="The differences are upside"
qflextable2(differences_between_options2)}
```

`r towrite`

The options used to create the initial data frame are: 

```{r}
s <-  differences_in_optionsinit_base%>% rename("Options" = X1, "Value" = X2)
qflextable2(s)
```
...
fonction_groupement = function(x, init, final){
  x  <-   enquo(x)
  groupement_1  <-   init %>% group_by(!!x,unit) %>% summarise(value_sum_1 = sum(value, na.rm=TRUE), number_lines1 = n()) %>%mutate(value_sum_1 = ifelse(is.na(value_sum_1), 0, value_sum_1))
  ...
  fulljoin  <-   full_join(groupement_1, groupement_2)%>% mutate(value_sum_2 = ifelse(is.na(value_sum_2), 0, value_sum_2)) %>%mutate(value_sum_1 = ifelse(is.na(value_sum_1), 0, value_sum_1))%>% mutate(loss = value_sum_1 - value_sum_2) %>% mutate(Loss_gain = ifelse(loss >= 0, "Loss", "Gain")) %>% 
    mutate(Loss_pourcent = 100*((value_sum_1 - value_sum_2)/value_sum_1))%>% mutate(Dimension = colnames(groupement_1[1])) %>%
    rename("Precision" = 1) %>% mutate(Precision = as.character(Precision)) %>% mutate(value_sum_2 = ifelse(is.na(value_sum_2), 0, value_sum_2))%>% mutate(Loss_pourcent = ifelse(is.na(Loss_pourcent)|Loss_pourcent==-Inf, 100, Loss_pourcent)) %>% mutate(loss_nb_ligne = number_lines1 - number_lines2)%>% 
    mutate(Loss_en_milliers_de_tonne = (value_sum_1 - value_sum_2)/ 10^6) %>% 
           rename(`Difference (in %)` = Loss_pourcent,`Difference in millions of tons` = Loss_en_milliers_de_tonne, 
                  `Difference in number of lines` = loss_nb_ligne) %>% mutate_if(is.numeric, list(~replace_na(., 0))) %>%  mutate(Loss_gain = case_when(is.na(Loss_gain) ~ "Gain",value_sum_1 == value_sum_2~"Egal", TRUE ~ Loss_gain))
}
```
...
strates_perdues <- t %>% filter(value_sum_2 == 0 & Loss_gain == "Loss") 
strates_gagnees <- t %>% filter(value_sum_1 == 0 & Loss_gain == "Gain")
...
strates_perdues_first_10 <- rbind(strates_perdues,strates_gagnees) %>% filter(Loss_gain != 'Egal')%>%ungroup %>% filter(Dimension!="geographic_identifier") %>% group_by(Loss_gain,Dimension)%>% slice(1:10) %>%select(Dimension, everything()) 

if (nrow(strates_perdues_first_10)== 0){strates_perdues_first_10 <- rbind(strates_perdues,strates_gagnees) %>% filter(Loss_gain != 'Egal')%>%ungroup %>% filter(Dimension!="geographic_identifier") %>% group_by(Loss_gain,Dimension)%>% slice(1:10) %>%select(Dimension, everything()) }
...
```{r}
if (nrow(t %>% filter(Loss_gain != 'Egal'))==0){
...
The differencies is `r loss_en_tonnes` in tons (**`r loss_en_tonnes_pourcent`%**). The differencies is `r loss_en_nombre` in number of fish (**`r ifelse(is.na(loss_en_nombre_pourcent), "0% (There was no data in number of fish neither in the first dataset nor in the second)",loss_en_nombre_pourcent) `**).

The stratas differencies between the first one and the second one are : 
...
if (nrow(strates_perdues_first_10)!=0){
  groupe_df <- as_grouped_data(strates_perdues_first_10%>% mutate(Loss_gain = ifelse(loss >= 0, "Loss", "Gain"))  %>%  mutate(Loss_gain = case_when(is.na(Loss_gain) ~ "Gain",value_sum_1 == value_sum_2~"Egal", TRUE ~ Loss_gain))%>% ungroup()%>% rename(`Captures table 1` = "value_sum_1",`Captures table 2` = "value_sum_2") %>% group_by(Dimension, unit,Loss_gain) %>% arrange(Dimension,unit,Loss_gain)%>% select(params$columns_to_keep)%>% mutate_if(is.numeric, round, digits=2), groups = c("Dimension","unit","Loss_gain"))

qflextable2(groupe_df)
towrite = ""} else {towrite =("No strata is gain nor lost")}
```
...
if(nrow(inner_join %>% filter(unit == variable_affichee)) != 0){


image <- tm_shape(inner_join %>% filter(unit == variable_affichee))+
...
This section detail the different differencies that observed between the dataframe `r titre_1` and `r titre_2`.
...
## The differencies for each dimension

We will look for each dimension the `r topn` most important differencies
...
unit <- t %>%  filter(Dimension %in% c("species", "source_authority","cat_geo", "fishingfleet", "species_group","gear", 
                                                                           "Name_En")) %>% mutate(Dimension = as.factor(Dimension)) %>%  group_by( Dimension, unit) %>% arrange(desc(`Difference (in %)`))   %>%   mutate(id = row_number())%>%   mutate(Precision = as.factor(ifelse(id>(topn-1),paste0("Others"),paste0(as.character(Precision))))) %>% ungroup() %>% group_by(Dimension,Precision, unit) %>% summarise(across(is.numeric, sum)) %>% mutate(`Difference (in %)` = (`Difference in millions of tons`*1000000/value_sum_1)*100) %>% select(-id, -number_lines2)%>%group_by(Dimension,unit) %>%  arrange(Dimension,unit,desc(`Difference (in %)`))

# %>% ungroup()%>% group_by(test,Dimension) %>% mutate_all(sum) %>% ungroup() %>% mutate(`Difference (in %)` =`Difference in millions of tons`/sum(`Difference in millions of tons`)) %>% mutate(Precision = test) %>%  mutate_if(is.character, as.factor)%>%select(-c(test,id)) %>%  group_by(Dimension) %>%arrange(Dimension,desc(`Difference (in %)`))

# unit_no <- t %>% filter(unit == "Number of fish") %>% filter(Dimension %in% c("species", "source_authority","cat_geo", "fishingfleet")) %>% mutate(Dimension = as.factor(Dimension)) %>%  mutate(test = ifelse(`Difference (in %)` >10,"Others",as.character(Precision))) %>%mutate(Precision = test) %>%    mutate(Loss_pourcent = 100*((value_sum_1 - value_sum_2)/value_sum_1)) %>%  mutate_if(is.character, as.factor)%>%select(-test) %>% ungroup() %>%  group_by(Precision,unit, Dimension)%>% summarise_all(sum)  %>% group_by(unit, Dimension) %>%arrange(desc(`Difference (in %)`), desc(`Difference in millions of tons`)) %>% group_by(Dimension)
# library(kable)
```


```{r paged.print=TRUE}
t3 <- as_grouped_data(unit %>% ungroup()%>% mutate(Loss_gain = ifelse(loss >= 0, "Loss", "Gain")) %>%  mutate(Loss_gain = case_when(is.na(Loss_gain) ~ "Gain",value_sum_1 == value_sum_2~"Egal", TRUE ~ Loss_gain))%>% rename(`Captures table 1` = "value_sum_1",`Captures table 2` = "value_sum_2")%>% select(params$columns_to_keep)%>% mutate_if(is.numeric, round, digits=2), groups = c("Dimension","unit"))
qflextable2(t3) 
```
...
## Differencies in geographical data

```{r testnaming}
tmap_mode("plot")
inner_join <- inner_join(shapefile.fix,(t %>% filter(Dimension == "geographic_identifier")) , by = c("code"="Precision"))%>% mutate(cat_loss = as.factor(case_when(`Difference (in %)` == 0 ~ 'No differencies', `Difference (in %)` == 100~'All data different',`Difference (in %)` < 0 ~ "Gain", TRUE ~ 'Loss')))

if (nrow(inner_join %>% filter(cat_loss == 'All data different'))!=0 & nrow(inner_join %>% filter(cat_loss == 'No differencies'))!=0){
image <- tm_shape(inner_join(shapefile.fix,(t %>% filter(Dimension == "geographic_identifier")) , by = c("code"="Precision"))) +
  tm_fill("Difference (in %)", palette="RdYlGn", style="cont", n=8,
				 id="name", midpoint = 0)+ tm_shape(inner_join %>% filter(cat_loss == 'No differencies')) + tm_borders(col="skyblue")+tm_shape(inner_join %>% filter(cat_loss == 'All data different')) +tm_borders(col="red")+tm_facets(by = c("unit", "cat_geo"),free.coords = TRUE, free.scales = TRUE)+
  tm_layout(legend.outside = FALSE)+tm_shape(continent)+tm_borders()
  tmap::tmap_save(image,"image2.png")
# mapshot(image, file = "image.png")
knitr::include_graphics("image2.png")} else{

image <- tm_shape(inner_join(shapefile.fix,(t %>% filter(Dimension == "geographic_identifier")) , by = c("code"="Precision"))) +
  tm_fill("Difference (in %)", palette="RdYlGn", style="cont", n=8,
				 id="name", midpoint = 0)+ tm_facets(by = c("unit", "cat_geo"),free.coords = TRUE, free.scales = TRUE)+
  tm_layout(legend.outside = FALSE) +tm_shape(continent)+tm_borders()
...
```{r testnaming2}
tmap::tmap_mode("plot")
image <- tm_shape(inner_join(shapefile.fix,(t %>% filter(Dimension == "geographic_identifier")) , by = c("code"="Precision")) %>% mutate(cat_loss = as.factor(case_when(`Difference (in %)` == 0 ~ 'No differencies', `Difference (in %)` == 100~'All data different' ,`Difference (in %)` < 0 ~ "Gain", TRUE ~ 'Loss')))
         # %>% filter(unit == "Tons")
         )+
  tm_fill( "cat_loss", palette="RdYlGn", style="cont", n=8,
				 id="name", midpoint = 0) +
  tm_layout(legend.outside = TRUE) + tm_facets(by = c("unit", "cat_geo"), free.scales = TRUE)+
...
## Differencies in temporal data

Here is represented the differencies in percent for each year. 
...
t %>%
 ...
 theme_minimal()+facet_wrap("unit")+labs(y = "Differencies in %")
...
if (nrow(setdiff_final_init) == 0| nrow(setdiff_init_final == 0)){knitr::knit_exit()} else{print("oncontinue")}
...
# The species differencies (to help for new analysis)

The data that was in initial table but not in final is: 

```{r}
setdiff_i_a <- setdiff(init, final) %>% select(-value) %>% left_join(init, by = c("fishingfleet", "gear", "time_start", "time_end", "geographic_identifier",
"schooltype", "species", "catchtype", "unit", "source_authority", "st_area", "cat_geo", "Name_En",
"species_group"))
```
...
```{r}
setdiff_a_i <- setdiff(final, init) %>% select(-value) %>% left_join(final, by = c("fishingfleet", "gear", "time_start", "time_end", "geographic_identifier",
"schooltype", "species", "catchtype", "unit", "source_authority", "st_area", "cat_geo", "Name_En",
"species_group"))
```
...
strates_perdues <- t %>% filter(`Difference (in %)` == 100)
nombre_strates_perdues <- nrow(strates_perdues)
nombres_de_strates_totales <- nrow(t)
pourcentage_strates_perdues <- 100-(100*((nombres_de_strates_totales-nombre_strates_perdues)/nombres_de_strates_totales))
rm(init_no, init_t, final_no, final_t)
...
The differencies is `r loss_en_tonnes` in tons (**`r loss_en_tonnes_pourcent`%**). The differencies is `r loss_en_nombre` in number of fish (**`r loss_en_nombre_pourcent`%**).
...
t4 <- head(strates_perdues%>% mutate(Loss_gain = ifelse(loss >= 0, "Loss", "Gain")) %>%  mutate(Loss_gain = case_when(is.na(Loss_gain) ~ "Gain",value_sum_1 == value_sum_2~"Egal", TRUE ~ Loss_gain))%>% rename(`Captures table 1` = "value_sum_1",`Captures table 2` = "value_sum_2")%>% select(params$columns_to_keep)%>% mutate_if(is.numeric, round, digits=2),10)


qflextable2(t4)
# kable(strates_perdues_first_10)
```
...
fishing_fleet <- pie_chart_2(fishingfleet, init, final)
source_authority <- pie_chart_2(source_authority, init, final)
try(cat_geo <- pie_chart_2(cat_geo, init, final))
species <- pie_chart_2(species, init, final)
species_group <- pie_chart_2(species_group, init, final)
# gear <- pie_chart_2(gear, init, final)
Name_En <- pie_chart_2(Name_En, init, final)
...
unit <- t%>%  filter(Dimension %in% c("species", "source_authority","cat_geo", "fishingfleet", "species_group",# "gear", 
                                                                           "Name_En")) %>% mutate(Dimension = as.factor(Dimension)) %>%  group_by( Dimension, unit) %>% arrange(desc(`Difference (in %)`))   %>%   mutate(id = row_number())%>%   mutate(Precision = as.factor(ifelse(id>(topn-1),paste0("Others"),paste0(as.character(Precision))))) %>% ungroup() %>% group_by(Dimension,unit, Precision) %>% summarise(across(is.numeric, sum)) %>% mutate(`Difference (in %)` = (`Difference in millions of tons`*1000000/value_sum_1)*100) %>% select(-id, -number_lines2)%>%group_by(unit,Dimension) %>%  arrange(Dimension,desc(`Difference (in %)`))
...
```{r paged.print=TRUE}
t5 <- as_grouped_data(unit %>% ungroup()%>% mutate(Loss_gain = ifelse(loss >= 0, "Loss", "Gain")) %>%  mutate(Loss_gain = case_when(is.na(Loss_gain) ~ "Gain",value_sum_1 == value_sum_2~"Egal", TRUE ~ Loss_gain))%>% rename(`Captures table 1` = "value_sum_1",`Captures table 2` = "value_sum_2")%>% select(params$columns_to_keep)%>% mutate_if(is.numeric, round, digits=2), groups = c("Dimension","unit"))
qflextable2(t5) 
```
...
image <- tm_shape(inner_join(shapefile.fix,(t %>% filter(Dimension == "geographic_identifier")) , by = c("code"="Precision")) %>% mutate(cat_loss = as.factor(case_when(`Difference (in %)` == 0 ~ 'No differencies', `Difference (in %)` == 100~'All data different', TRUE ~ 'Partial differencies')))
         # %>% filter(unit == "Tons")
         )+
  tm_fill( "value", palette="RdYlGn", style="cont", n=8,
				 id="name", midpoint = 0) +
  tm_layout(legend.outside = TRUE) + tm_facets(by = c("unit", "cat_geo"))
  tmap::tmap_save(image, paste0(getwd(),"image5.png"))
# mapshot(image, file = "image.png")
knitr::include_graphics(paste0(getwd(),"image5.png"))
