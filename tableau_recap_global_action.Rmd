...
options(knitr.duplicate.label = "allow")#for duplicate chunk names
```
...
qflextable2 =function(x, captionn = NULL, autonumm = autonum, pgwidth = 6, colour = FALSE){
  ...
  y <- y %>% mutate_if(is.factor, as.character) %>% mutate_if(is.character, ~str_replace_all( ., ",", ", \n" )) %>% 
  mutate_if(is.character,~str_replace_all( ., "_", "-" )) %>% mutate_if(is.numeric, function(.){round(.,2)})
