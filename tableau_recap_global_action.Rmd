...
try(res2 <- knitr::knit_child("Analyse_georeferenced_child.Rmd", envir = globalenv(), quiet = TRUE))
```
...
try(res <- knitr::knit_child("comp_sans_shiny_child.Rmd", envir = globalenv(), quiet = TRUE))
...
try(all <- lapply(1:(length(sub_list_dir_2)-1), function_all_comp))
...
try(cat(unlist(all), sep = paste0( "\\clearpage")))
