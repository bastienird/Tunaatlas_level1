...
join <- full_join(init_group, final_group) %>% dplyr::arrange(unit) %>%dplyr::rowwise() %>%  dplyr::mutate(unit = paste0("Value in ", unit)) %>% dplyr::mutate(Difference = titre_test1 - titre_test2)%>% dplyr::rename(!!as.symbol(eval(parse(text = "titre_1"))) := titre_test1)%>% dplyr::rename(!!as.symbol(eval(parse(text = "titre_2"))) := titre_test2)
