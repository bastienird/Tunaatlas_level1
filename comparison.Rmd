...
groupement_1  <-   init %>%dplyr::ungroup() %>%  dplyr::group_by(!!!rlang::syms(these_col)
  #!!these_col
  ,unit) %>% dplyr::summarise(value_sum_1 = round((sum(value, na.rm=TRUE)))) %>% dplyr::ungroup() %>%dplyr::mutate(value_sum_1 = dplyr::coalesce(value_sum_1, 0))%>% dplyr::mutate(unit = as.character(unit))%>% dplyr::mutate(number_lines1 = n())
...
groupement_2  <-   final %>%dplyr::ungroup() %>%  dplyr::group_by(!!!rlang::syms(these_col)
  #!!these_col
  ,unit) %>%  dplyr::summarise(value_sum_2 = round((sum(value, na.rm=TRUE)))) %>% dplyr::ungroup()  %>%dplyr::mutate(value_sum_2 = dplyr::coalesce(value_sum_2, 0)) %>% dplyr::mutate(unit = as.character(unit)) %>% dplyr::mutate(number_lines2 = n())
