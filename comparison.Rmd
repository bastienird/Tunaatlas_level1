...
params: 
  ...
  final: "~/Documents/Tunaatlas_level1/jobs/20221201182627/entities/global_catch_1deg_level0__final_test_new_dcmi_test23/Markdown/Level2_RF3without_gears"
  dbname: ""
  host: ""
  port: 
  user: ""
...
last_path_reduced = function(x){base::gsub("georef_dataset","",last_path(x))}
...
parameter_filtering <- if(!is_null_or_not_exist(action$options$filtering)){action$options$filtering} else{ list(species = NULL, fishingfleet = NULL)}
...
if(!require(sutdycatchesird)){
  ...
  require(devtools)
  out <- try(devtools::install_local("~/Documents/studycatchesird"))
...
titre_2 <- base::gsub("_","-",titre_2)
titre_1 <- base::gsub("_","-",titre_1)
...
mapping_dataset <- read_csv(paste0("data/", entity$data$source[[2]])) #bof mais pour l'instant comme ça 
  ...
  init <- map_codelistsB(con, parameter_fact, mapping_dataset, init, mapping_keep_src_code = FALSE, summary_mapping = FALSE) #this map condelist function is to retrieve the mapping dataset used
...
differences_in_optionsinit$X1 <- base::gsub(" ","",differences_in_optionsinit$X1)
differences_in_optionsinit$X1 <- base::gsub("options_","",differences_in_optionsinit$X1) 
differences_in_optionsinit$X2 <- base::gsub(" ","",differences_in_optionsinit$X2)
differences_in_optionsinit$X2 <- base::gsub("options_","",differences_in_optionsinit$X2) 
...
differences_in_optionsfinal$X1 <- base::gsub(" ","",differences_in_optionsfinal$X1)
differences_in_optionsfinal$X1 <- base::gsub("options_","",differences_in_optionsfinal$X1)
differences_in_optionsfinal$X2 <- base::gsub(" ","",differences_in_optionsfinal$X2)
differences_in_optionsfinal$X2 <- base::gsub("options_","",differences_in_optionsfinal$X2)
...
s <-  differences_in_optionsinit_base%>% dplyr::rename("Options" = X1, "Value" = X2) %>% dplyr::mutate(Options = base::gsub("_","-",Options))%>% dplyr::mutate(Options = base::gsub("=","=\n",Options))%>% dplyr::mutate(Value = base::gsub("_","-",Value))%>% dplyr::mutate(Value = base::gsub("=","=\n",Value)) %>% dplyr::mutate(Options = base::gsub("NONE", "", Options))
```
...
if(unique_analyse){title_introduction <- " "}
```
...
assign("Dimension2", base::gsub("_", "", unique(x$data$Dimension)), envir = environment())
...
map_unit_knit = function(map){
  ...
  knitr::knit_child(text = c(
    ...
    '',
    '```{r}', 
    'dimension_title <- base::gsub("_","..",unit_name_map)',
    '```',
    '',
...
resume_knit_child = function(x){
  ...
  knitr::knit_child(text = c(
    '',
    '```{r}', 
    'dimension_title <- base::gsub("_","..",x$dimension)',
    '',
    '```',
    '',
    '',
...
```{r echo=FALSE, eval=unique_analyse, fig.cap='Other dimensions', fig.subcap=c(unlist(base::gsub("_","..",dimension_title_subfigures))), fig.ncol = 2, out.width = "50%", fig.align = "center"}
...
knit_child_map =function(list_map_unit){
  ...
  knit_child(text = c(
    '```{r results = "asis"}',
    '',
    'unit_title <- base::gsub("_","..",unit_name_map)',
    '```',
    '',
    '',
