...
params: 
  ...
  final: "~/Documents/Tunaatlas_level1/jobs/20220927110301_global_catch/entities/global_catch_1deg_level1_final/Markdown/Treatment_iattc_iccat"
  dbname: ""
  host: ""
  port: 
  user: ""
...
conflicted::conflict_prefer_all("dplyr", losers = NULL, quiet = FALSE)
...
return(dataframe_to_filter %>% dplyr::rename("geographic_identifier" := {{geo_dim}}))}
}
...
join <- dplyr::full_join(init_group, final_group) %>% dplyr::arrange(unit) %>%dplyr::rowwise() %>%  dplyr::mutate(unit = paste0("Value in ", unit)) %>% dplyr::mutate(Difference = titre_test1 - titre_test2)%>% dplyr::rename(!!as.symbol(eval(parse(text = "titre_1"))) := titre_test1)%>% dplyr::rename(!!as.symbol(eval(parse(text = "titre_2"))) := titre_test2)
...
```{r}
# groupe_df %>% group_by(unit, Dimension) %>%rowwise() %>%  summarise(value_sum1 = nrow(unique(Precision)), value_sum2 = nrow(unique(Precision)))
...
s <- cbind(number_init_column , number_final_column )
...
if(!(require(tibble))){ 
  ...
  (require(tibble))} 
set_caption(qflextable2(s %>% tibble::rownames_to_column(" ")), caption = "Comparison of number of stratas between the two datasets",  style = "Table Caption")
```
