...
if(is_null_or_not_exist(parameter_colnames_to_keep)){colnames_to_keep <- c("fishingfleet",         "gear_type",                 "time_start",                 
...
Dimensions <- colnames(init)[colnames(init) != "measurement_unit" & colnames(init)!= "measurement_value"& colnames(init)!= "st_area"]
t <- carte_init[0,]
...
fonction_empreinte_spatiale = function(variable_affichee){
  selection = function(x){x %>% dplyr::ungroup() %>% dplyr::select(geographic_identifier, measurement_value, cat_geo, measurement_unit)}
  ...
  inner_join <- st_as_sf(geo_data%>% dplyr::group_by(geographic_identifier, measurement_unit,source)  %>%  dplyr::summarise(measurement_value = sum(measurement_value, na.rm = TRUE))  %>% dplyr::filter(measurement_value != 0) %>% dplyr::inner_join(shapefile.fix, by = c("geographic_identifier"="code")))
  ...
  if(nrow(inner_join %>% dplyr::filter(measurement_unit == variable_affichee)) != 0){
     if (plotting_type == "view"){image <- tm_shape(inner_join %>% dplyr::filter(measurement_unit == variable_affichee))+
      tm_fill("measurement_value", palette="RdYlGn", style="cont", n=8,
              id="name", midpoint = 0) +
      tm_layout(legend.outside = FALSE) + tm_facets(by=c("cat_geo","source"), free.scales = TRUE)} else {image <- tm_shape(inner_join %>% dplyr::filter(measurement_unit == variable_affichee))+
        tm_fill("measurement_value", palette="RdYlGn", style="cont", n=8,
                id="name", midpoint = 0) +
...
if (nrow(inner_join %>%dplyr::filter(`Impact on the data` == 'All data different'))!=0 & nrow(inner_join %>%dplyr::filter(`Impact on the data` == 'No differences'))!=0){
  if(plotting_type == "plot"){
    image <- tm_shape(dplyr::inner_join(shapefile.fix,(t %>%dplyr::filter(Dimension == "geographic_identifier")) , by = c("code"="Precision")) %>% dplyr::filter("Difference (in %)" != 100 & "Difference (in %)" != 0 )) +
      tm_fill("Difference (in %)", palette="RdYlGn", style="cont", n=8,
              id="name", midpoint = 0)+ tm_shape(inner_join %>%dplyr::filter(`Impact on the data` == 'No differences')) + tm_borders(col="skyblue")+tm_shape(inner_join %>%dplyr::filter(`Impact on the data` == 'All data different')) +tm_borders(col="red")+tm_facets(by = c("measurement_unit", "cat_geo"),free.coords = FALSE, free.scales = FALSE)+
  ...
  image <- tm_shape(dplyr::inner_join(shapefile.fix,(t %>%dplyr::filter(Dimension == "geographic_identifier")) , by = c("code"="Precision"))%>% dplyr::filter("Difference (in %)" != 100 & "Difference (in %)" != 0 )) +
    tm_fill("Difference (in %)", palette="RdYlGn", style="cont", n=8,
            id="name", midpoint = 0)+ tm_facets(by = c("measurement_unit", "cat_geo"),free.coords = FALSE, free.scales = FALSE)+
...
t_filtered <- dplyr::inner_join(shapefile.fix,(t %>%dplyr::filter(Dimension == "geographic_identifier")) , by = c("code"="Precision")) %>% dplyr::group_by(measurement_unit,cat_geo) %>%  dplyr::mutate(`Difference (in %)` = log(`Difference (in %)` + abs(min(`Difference (in %)`)) +1)) %>% dplyr::filter(!(value_sum_1 ==0 & value_sum_2==0))
if (nrow(inner_join %>%dplyr::filter(`Impact on the data` == 'All data different'))!=0 & nrow(inner_join %>%dplyr::filter(`Impact on the data` == 'No differences'))!=0){
  if(plotting_type == "plot"){
    image <- tm_shape(t_filtered)  +
      tm_fill("Difference (in %)",style = "equal", palette = "Blues", auto.palette.mapping = FALSE, midpoint = 0)+tm_facets(by = c("measurement_unit", "cat_geo"),free.coords = FALSE, free.scales = FALSE)+
      tm_layout(legend.outside = TRUE)+tm_shape(continent)+tm_borders()
    } else {
      image <- tm_shape(t_filtered) +
  ...
  image <- tm_shape(t_filtered%>% dplyr::filter("Difference (in %)" != 100 & "Difference (in %)" != 0 )) +
    tm_polygons(col = "Difference (in %)", palette="PiYG", style="cont", n=8,
            id="name", midpoint = 0,interval.closure	= "left")+ tm_facets(by = c("measurement_unit", "cat_geo"),free.coords = FALSE, free.scales = FALSE)+
    tm_layout(legend.outside = TRUE) +tm_shape(continent)+tm_borders()
...
breaks$`Impact on the data` <- factor(breaks$`Impact on the data`, levels = c("Appearing data", "Gain (more than double)","Gain", "No differences", "Loss", "All data lost"))
 ...
 image <- tm_shape(breaks)+tm_fill("Impact on the data", palette="-PiYG",  id="name")+ tm_facets(by = c("measurement_unit", "cat_geo"), free.scales = FALSE,free.coords = TRUE)+    tm_layout(legend.outside = TRUE)
...
out <- try(spatial_coverage_init <-sum((init %>% dplyr::filter(measurement_value > 0)%>% select(geographic_identifier)%>%dplyr::distinct()%>%dplyr::left_join(shape_without_geom %>% mutate(st_area=as.numeric(st_area)), by = c("geographic_identifier"="code")) %>% distinct())$st_area, na.rm = TRUE))
...
out <- try(spatial_coverage_final <- as.numeric(sum((final %>% dplyr::filter(measurement_value > 0)%>% select(geographic_identifier)%>%dplyr::distinct() %>%dplyr::left_join(shape_without_geom%>% mutate(st_area=as.numeric(st_area)), by = c("geographic_identifier"="code")) %>% distinct)$st_area, na.rm = TRUE)))
...
fonction_empreinte_spatiale_diff  = function(unit, plotting_type, shapefile_fix_function, t_filtered, percent = FALSE){
  ...
  dataset <- dplyr::inner_join(shapefile_fix_function,(t_filtered%>% dplyr::ungroup() %>%dplyr::filter(Dimension == ("geographic_identifier")) %>% dplyr::filter(measurement_unit == !!enquo_unit)) , by = c("code"= "Precision"))
  if(percent){
