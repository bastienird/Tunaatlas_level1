...
if(is_null_or_not_exist(columns_to_keep)){
  parameter_columns_to_keep <- c("Precision", "unit", "Values dataset 1",
...
```{r GTA_MARKDOWN_FUNCTIONS, include=FALSE}
...
tidying_data_for_GTA = function(dataframe, time_dim, shape, species_group_dataframe = species_group, cl_cwp_gear_level2_dataframe = cl_cwp_gear_level2){
  ...
  dataframe <- dataframe%>%  dplyr::left_join(shape, by = c("geographic_identifier"="code"))
  print_map <- TRUE
...
if("cat_geo"%in%colnames(dataframe)){
  dataframe <- dataframe%>%dplyr::mutate(cat_geo = as.character(cat_geo))
}

dataframe <- dataframe %>% dplyr::left_join(species_group_dataframe%>% dplyr::distinct(), by = c("species"))
  
    dataframe <- dataframe %>% dplyr::left_join(cl_cwp_gear_level2_dataframe, by = c("gear" = "Code"))
    
  if ("Name_En"%in% colnames(dataframe)){dataframe <- dataframe%>% dplyr::rename(Gear = Name_En)%>%dplyr::mutate(gear = as.character(gear))}
  
  dataframe <- dataframe%>%dplyr::mutate(unit = dplyr::case_when(unit %in% c("MT","t","MTNO", "Tons")~ "Tons", unit %in% c("NO", "NOMT","no", "Number of fish")~"Number of fish", TRUE ~ unit)) 
...
fonction_groupement = function(these_col, init, final){
  ...
  # Compute sum of values for each combination of the columns in "these_col" and "unit" in the "init" dataframe
  groupement_1 <- init %>%
    dplyr::ungroup() %>%
    dplyr::group_by(!!!rlang::syms(these_col), unit) %>%
    dplyr::summarise(value_sum_1 = ((sum(value, na.rm=TRUE)))) %>%
    dplyr::ungroup() %>%
    dplyr::mutate(value_sum_1 = dplyr::coalesce(value_sum_1, 0)) %>%
    dplyr::mutate(unit = as.character(unit)) %>%
    dplyr::mutate(number_lines1 = n())
  
  # Compute sum of values for each combination of the columns in "these_col" and "unit" in the "final" dataframe
  groupement_2 <- final %>%
    dplyr::ungroup() %>%
    dplyr::group_by(!!!rlang::syms(these_col), unit) %>%
    dplyr::summarise(value_sum_2 = ((sum(value, na.rm=TRUE)))) %>%
    dplyr::ungroup() %>%
    dplyr::mutate(value_sum_2 = dplyr::coalesce(value_sum_2, 0)) %>%
    dplyr::mutate(unit = as.character(unit)) %>%
    dplyr::mutate(number_lines2 = n())
  
  # Join the two dataframes based on the columns in "these_col" and "unit"
...
```{r species_and_gear, cache=FALSE}
...
if(is_null_or_not_exist(parameter_colnames_to_keep)){colnames_to_keep <- c("fishingfleet",         "gear",                 "time_start",                 
                      "geographic_identifier","schooltype",           "species",                       
...
mapping_dataset <- read.csv(paste0("data/codelist_mapping_rfmos_to_global.csv"), stringsAsFactors = F,colClasses = "character")#, entity$data$source[[2]])) #bof mais pour l'instant comme ça 
  ...
  init <- map_codelists(con = con, parameter_fact = parameter_fact, mapping_dataset = mapping_dataset, dataset_to_map = init, mapping_keep_src_code = FALSE, summary_mapping = FALSE)$dataset_mapped #this map condelist function is to retrieve the mapping dataset used
...
```{r shapefilereading, eval = print_map, include=FALSE}
...
```{r explanation_on_gain_and_loss, results='asis', eval=(!parameter_short && !unique_analyse)}
...
```{r explanation_on_result_mapping, results='asis', eval=(!parameter_mapped &&  !unique_analyse) }
...
```{r treatment, eval = treatment, include=FALSE}
...
```{r include=FALSE}
if(exists("no_recap_options") && no_recap_options == TRUE){options_total <- FALSE} else {options_total <- TRUE}
```


```{r eval=treatment && options_total}
...
```{r summarytreatment, results='asis', eval=(treatment && !last_to_first)}
...
```{r eval=(!parameter_short && !unique_analyse & options_total)}
...
```{r results='asis', eval=(!parameter_short && !unique_analyse & options_total)}
cat(towrite)
```
...
```{r allfonction_groupement,  message=FALSE, warning=FALSE, include=FALSE}
for (i in Dimensions){
...
```{r summary_of_differences, eval=!unique_analyse}
...
```{r stratas_loss, echo=FALSE, eval=!unique_analyse, message=FALSE, warning=FALSE, paged.print=TRUE, results='asis'}
...
if (nrow(strates_perdues_first_10)!=0){
  ...
  groupe_df <- flextable::as_grouped_data(strates_perdues_first_10 %>%dplyr::mutate(`Loss / Gain` = dplyr::case_when(loss > 1~ "Loss", abs(loss)<=1 ~ "No differences", loss < 1~ "Gain", TRUE ~ NA_character_))  %>% dplyr::mutate(`Loss / Gain` = dplyr::case_when(is.na(`Loss / Gain`) ~ "Gain",value_sum_1 == value_sum_2~"Egal", TRUE ~ `Loss / Gain`))%>% dplyr::ungroup()%>%  dplyr::rename(`Values dataset 1` = "value_sum_1",`Values dataset 2` = "value_sum_2") %>% dplyr::select(parameter_columns_to_keep) %>%  dplyr::group_by(Dimension, unit,`Loss / Gain`) %>% dplyr::arrange( Dimension, unit,`Loss / Gain`,desc(`Difference in value`)) %>% ungroup(),groups = c("Dimension","unit","Loss / Gain"))
  cat("The stratas differences (completely lost or appearing) between the first one and the second one are:\n")
  cat(knit_print(qflextable2(groupe_df%>% dplyr::select(Dimension, unit, `Loss / Gain`, Precision, `Difference in millions` = `Difference in value`), captionn = paste0("Disappearing or appearing stratas between ", titre_1, " and ", titre_2))))
...
} else {dimension_title_subfigures <- c("NULL","NULL")}
...
if(nrow(unit) != 0){
  ...
  unit <- unit%>%dplyr::mutate(`Loss / Gain` = base::ifelse(loss >= 0, "Loss", "Gain")) %>% dplyr::mutate(`Loss / Gain` = dplyr::case_when(is.na(`Loss / Gain`) ~ "Gain", value_sum_1 == value_sum_2 ~"Egal", TRUE ~ `Loss / Gain`))%>%dplyr::mutate(Dimension = as.factor(Dimension)) %>%  dplyr::group_by(Dimension, unit,`Loss / Gain`) %>% arrange(desc(`Difference (in %)`)) %>% dplyr::mutate(id = row_number())%>%  dplyr::mutate(Precision = as.factor(base::ifelse(id>(topn),paste0("Others"),paste0(as.character(Precision))))) %>% dplyr::ungroup() %>%dplyr::group_by(`Loss / Gain`,Dimension,Precision, unit) %>%dplyr::summarise(across(is.numeric, sum)) %>%dplyr::mutate(`Difference (in %)` = (`Difference in value`/value_sum_1)*100) %>%dplyr::select(-id, -number_lines2)%>%dplyr::group_by(`Loss / Gain`,Dimension,unit) %>%  arrange(Dimension,unit,desc(`Difference (in %)`)) %>% mutate(Other_column = ifelse(Precision == "Others", 1, 0)) 
...
t3 <- as_grouped_data(unit %>% dplyr::ungroup() %>% 
                      ...
                      dplyr::group_by(Dimension, unit, `Loss / Gain`) %>% 
                      dplyr::arrange(Dimension, unit, `Loss / Gain`, Other_column, desc(abs(`Difference in value`))) %>% 
