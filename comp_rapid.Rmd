...
params: 
  final: "~/Documents/Tunaatlas_level1/jobs/difference_L1_L2/entities/global_catch_5deg_1m_IRD_level2/Markdown/endlevel0data/rds.rds"
  init: "~/Documents/Tunaatlas_level1/jobs/no_difference_L1_L2/entities/global_catch_5deg_1m_ird_level2/Markdown/OverlappingIATTCWCPFC/rds.rds"
  filter_species:  "NULL"
  ...
  titre_dataset_1: "endlevel0noraising"
  titre_dataset_2: "endlevel0raising"
...
init <- fonction_filtre(readRDS(as.character(params$init)) %>% select(all_of(colnames_to_keep)))%>% inner_join(species_group, by = c("species"))


final <- fonction_filtre(readRDS(as.character(params$final))%>% select(all_of(colnames_to_keep)))%>% inner_join(species_group, by = c("species"))
