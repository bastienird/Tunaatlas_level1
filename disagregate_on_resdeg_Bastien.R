function_spatial_curation_upgrade_Bastien = function (con, df_input, resolution, remove = FALSE) {
  ...
  if (resolution == 5) {
    ...
    if (nrow(areas_to_project_data_to_aggregate) > 0) {
      ...
      dataset_areas_inf_to_resolution_to_disaggregate <- df_input %>% 
  ...
  dataset_to_leave_as_so <- inner_join(df_input, cwp_grid_data_with_resolution_to_downgrade, 
                                       by = "geographic_identifier")
  ...
  gc()
  return(list(df = dataset_final_disaggregated_on_resolution_to_disaggregate, 
