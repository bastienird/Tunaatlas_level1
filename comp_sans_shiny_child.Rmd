...
if (nrow(strates_perdues_first_10)!=0){
  groupe_df <- flextable::as_grouped_data(strates_perdues_first_10 %>% filter(Dimension %notin%c("time_start", "time_end"))%>%dplyr::mutate(`Loss / Gain` = ifelse(loss >= 0, "Loss", "Gain"))  %>% dplyr::mutate(`Loss / Gain` = case_when(is.na(`Loss / Gain`) ~ "Gain",value_sum_1 == value_sum_2~"Egal", TRUE ~ `Loss / Gain`))%>% dplyr::ungroup()%>%  dplyr::rename(`Captures table 1` = "value_sum_1",`Captures table 2` = "value_sum_2") %>% select(parameter_columns_to_keep) %>%  dplyr::group_by(Dimension, unit,`Loss / Gain`) %>% dplyr::arrange( Dimension, unit,`Loss / Gain`,desc(`Difference in millions of tons/fish`)),groups = c("Dimension","unit","Loss / Gain"))
...
knit_child_map =function(unit, plotting_type, shapefile_fix_function, t_filtered, percent = FALSE){
  ...
  knit_child(text = c(
    ...
    'unit_title <- gsub("_","..",unit)',
     'cat2(paste0("### For the unit : ", unit,  " \n \n  "))',
    '```',
    '',
    '',
    '```{r out.width="100%", results="asis", fig.cap = paste0("Map of the differences between two datasets in millions of ", unit_title)}',
'',
  'image_child',
  '',
    '```',
...
cat(unlist(restest_percent), sep = " \n ")
...
if(plotting_type == "plot"){
  image <- tm_shape(inner_join(shapefile.fix,(t %>%dplyr::filter(Dimension == "geographic_identifier")) , by = c("code"="Precision")) %>%dplyr::mutate(`Impact on the data` = as.factor(case_when(value_sum_1 == value_sum_2 ~ 'No differences', `value_sum_2` == 0 & value_sum_1 != 0 ~'All data lost' ,`Difference (in %)` < 0 ~ "Gain", TRUE ~ 'Loss')))
    ...
    tm_fill( "Difference (in %)", palette="-PiYG", style="fixed", breaks = c(-Inf, -100,0,0,100,100),labels = c("Gain (more than double)","Gain", "No differences","Loss", "All data lost" ),
    ...
    tm_fill( "Difference (in %)", palette="-PiYG", style="fixed", breaks = c(-Inf, -100,0,0,100,100),labels = c("Gain (more than double)","Gain", "No differences","Loss", "All data lost" ),
               id="name", midpoint = 0)+tm_borders(lwd = 0.005)+ tm_facets(by = c("unit", "cat_geo"), free.scales = FALSE,free.coords = TRUE)+
      tm_layout(legend.outside = TRUE)}
...
```{r out.width="100%", fig.cap=paste0(" Difference (in %) between ", titre_1, " and ", titre_2)}
