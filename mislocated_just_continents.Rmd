...
date: '2022-03-14'
params:
  chemin_dacces: "~/Documents/Tunaatlas_level1/jobs/20220511123321/entities/global_catch_1deg_1m_ps_bb_firms_Bastien_with_step_rds__level2/Markdown/rawdata/rds.rds"
  plotting: "view"
...
# if (!is.null(params$source_authority)) {data <- data%>% filter(source_authority == params$source_authority)}
...
world_sf <- fSpatPlan_Convert2PacificRobinson(st_make_valid(st_read(con, query = query))%>% filter(!st_is_empty(.)))
...
IOTC_shape <- fSpatPlan_Convert2PacificRobinson(competence_area %>% filter(code == "IOTC"))
IATTC_shape <- fSpatPlan_Convert2PacificRobinson(competence_area %>% filter(code == "IATTC"))
WCPFC_shape <- fSpatPlan_Convert2PacificRobinson(competence_area %>% filter(code == "WCPFC"))
ICCAT_shape <- fSpatPlan_Convert2PacificRobinson(competence_area %>% filter(code == "ICCAT"))
...
irregular_iotc <- fSpatPlan_Convert2PacificRobinson(st_read(con, query = query)%>% filter(!st_is_empty(.)))
...
tm_shape(st_as_sf(mislocated_continent %>% mutate(st_area = as.factor(st_area))))+tm_fill("value")+tm_facets(by=c("st_area","unit"),free.scales = FALSE)+tm_borders()+tm_text("source_authority")#+tm_layout(legend.outside	 =TRUE)#+  tm_shape(continent)+tm_borders() 
...
mislocated_continent_time <- mislocated_cwp %>% mutate(time_start = lubridate::year(time_start)) %>% group_by(unit, time_start) %>% summarise(value = sum(value))
...
pie_chart_2(fishingfleet, mislocated_cwp %>% filter(unit == "Tons"), data%>% filter(unit == "Tons"))
...
```{r}
pie_chart_2(fishingfleet, mislocated_cwp %>% filter(unit == "Number of fish"), data%>% filter(unit == "Number of fish"))
```
...
```{r}
pie_chart_2(species, mislocated_cwp %>% filter(unit == "Tons"), data%>% filter(unit == "Tons"))
```
