...
output:
  ...
  bookdown::html_document2:
    ...
    number_sections: TRUE
params: 
  init: "~/Documents/Tunaatlas_level1/jobs/20221019232032/entities/global_catch_1deg_level0__final_test/Markdown/rawdata"
...
```{r}
write(parameter_fact, "parameter_fact.txt")
```
...
if(!(require(data.table))){ 
  ...
  (require(data.table))}
fwrite(head(init,50), "init_50.csv")
...
```{r results='asis', eval=parameter_mapped }
...
fonction_groupement = function(these_col, init, final){
  fwrite(head(init), "init_2.csv")
  if(!(require(rlang))){ 
  install.packages("rlang") 
  (require(rlang))} 
# groupement_1 <- init[, value := lapply(.SD, sum), by = these_col, .SDcols = c("value")]
# groupement_1 <- groupement_1 %>% mutate(number_lines1 = 0)%>%dplyr::mutate(value_sum_1 = dplyr::coalesce(value_sum_1, 0))%>% mutate(unit = as.character(unit))
  # these_col <- enquo(these_col) 
groupement_1  <-   init %>%ungroup() %>%  dplyr::group_by(!!!rlang::syms(these_col)
  #!!these_col
  ,unit) %>% dplyr::summarise(value_sum_1 = round((sum(value, na.rm=TRUE)))) %>% ungroup() %>%dplyr::mutate(value_sum_1 = dplyr::coalesce(value_sum_1, 0))%>% mutate(unit = as.character(unit))%>% dplyr::mutate(number_lines1 = n())
fwrite(groupement_1, "groupement_1.csv")
fwrite(head(final), "final_2.csv")

# groupement_2 <- final[, value := lapply(.SD, sum), by = these_col, .SDcols = "value"]
# groupement_2 <- groupement_2 %>% mutate(number_lines2 = 0)%>%dplyr::mutate(value_sum_2 = dplyr::coalesce(value_sum_2, 0))%>% mutate(unit = as.character(unit))

groupement_2  <-   final %>%ungroup() %>%  dplyr::group_by(!!!rlang::syms(these_col)
  #!!these_col
  ,unit) %>%  dplyr::summarise(value_sum_2 = round((sum(value, na.rm=TRUE)))) %>% ungroup()  %>%dplyr::mutate(value_sum_2 = dplyr::coalesce(value_sum_2, 0)) %>% mutate(unit = as.character(unit)) %>% dplyr::mutate(number_lines2 = n())
fwrite(groupement_2, "groupement_2.csv")

fulljoin  <-   dplyr::full_join(groupement_1, groupement_2) %>%  dplyr::mutate(value_sum_2 = dplyr::coalesce(value_sum_2, 0)) %>%dplyr::mutate(value_sum_1 = dplyr::coalesce(value_sum_1, 0))%>%dplyr::mutate(loss = value_sum_1 - value_sum_2) %>%dplyr::mutate(`Loss / Gain` = dplyr::case_when(loss == 0 ~ "Egal", loss > 0 ~"Loss",  loss < 0 ~"Gain")) %>%
dplyr::mutate(Loss_pourcent = 100*((value_sum_1 - value_sum_2)/value_sum_1))%>%dplyr::mutate(Dimension = colnames(groupement_1[1])) %>%
dplyr::rename("Precision" = 1) %>%dplyr::mutate(Precision = as.character(Precision)) %>%dplyr::mutate(value_sum_2 = dplyr::coalesce(value_sum_2, 0))%>%dplyr::mutate(Loss_pourcent = base::ifelse(is.na(Loss_pourcent)|Loss_pourcent==-Inf, 100, Loss_pourcent)) %>% ungroup() %>%  dplyr::mutate(loss_nb_ligne = number_lines1 - number_lines2)%>%
...
fonction_groupement = function(x, init, final){
  these_col <- enquo(x) 
groupement_1  <-   init %>% dplyr::group_by(#!!!rlang::syms(x)
  !!these_col,unit) %>% dplyr::summarise(number_lines1 = 0,
value_sum_1 = round((sum(value, na.rm=TRUE)))) %>%dplyr::mutate(value_sum_1 = dplyr::coalesce(value_sum_1, 0))%>% mutate(unit = as.character(unit))
fwrite(groupement_1, "groupement_1.csv")

groupement_2  <-   final %>% dplyr::group_by(#!!!rlang::syms(x)
  !!these_col,unit) %>%  dplyr::summarise(number_lines2 = 0,
value_sum_2 = round((sum(value, na.rm=TRUE)))) %>%dplyr::mutate(value_sum_2 = dplyr::coalesce(value_sum_2, 0)) %>% mutate(unit = as.character(unit))
fwrite(groupement_1, "groupement_2.csv")
...
fwrite(head(final), "final_3.csv")
fwrite(head(init), "init_3.csv")
...
fwrite(head(carte_init), "head_cart_init.csv")
```
...
set_caption(join, caption = "Summary of the difference between the two datasets",  style = "Table Caption")
...
```{r test}
kable(summary(time_dimension_list_groupped_diff))
kable(summary(time_dimension_list_groupped_diff[1]))
qflextable2(as.data.frame(summary(time_dimension_list_groupped_diff[[1]] %>% mutate_if(is.character, as.factor))))
```
...
breaks <- inner_join(shapefile.fix,(t %>%dplyr::filter(Dimension == "geographic_identifier")) , by = c("code"="Precision")) %>%dplyr::mutate(`Impact on the data` = dplyr::case_when(`Difference (in %)` == -Inf ~"Appearing data", -Inf<`Difference (in %)`  & `Difference (in %)`<= -100 ~"Gain (more than double)" ,  -100<`Difference (in %)`  & `Difference (in %)`<0 ~ "Gain",`Difference (in %)`==0~"No differences" ,0<`Difference (in %)`  & `Difference (in %)`<100 ~ "Loss",`Difference (in %)` ==100 ~  "All data lost" ))
 ...
 image <- tm_shape(breaks)+tm_fill( "Impact on the data", palette="-PiYG",  id="name", midpoint = 0)+tm_borders(lwd = 0.05)+ tm_facets(by = c("unit", "cat_geo"), free.scales = FALSE,free.coords = TRUE, drop.units = FALSE)+    tm_layout(legend.outside = TRUE)
