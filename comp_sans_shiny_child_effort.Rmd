...
`Difference in number of lines` = loss_nb_ligne)  %>% dplyr::mutate(`Loss / Gain` = dplyr::case_when(is.na(`Loss / Gain`) ~ "Gain",round(`Difference in value`) == 0~"Egal", TRUE ~ `Loss / Gain`))%>%dplyr::mutate_if(is.numeric, list(~replace_na(., 0)))
}
```
...
```{r results='asis', eval=!unique_analyse, out.width="100%"}
