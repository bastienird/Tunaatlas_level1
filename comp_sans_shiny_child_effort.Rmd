...
output:
  ...
  bookdown::html_document2:
    ...
    number_sections: TRUE
params: 
  init: "~/Documents/Tunaatlas_level1/jobs/20220927110301_global_catch/entities/global_catch_1deg_level1_final/Markdown/rawdata"
...
return(dataframe_to_filter %>% rename("geographic_identifier" := {{geo_dim}}))
}
...
if(unique_analyse){final <- init[0,] %>% mutate(unit = as.character(unit))} else {final <- filtering_function(final, colnames_to_filter = colnames_to_filter, matchingList = matchingList)}
...
if (nrow(strates_perdues_first_10)!=0){
  groupe_df <- flextable::as_grouped_data(strates_perdues_first_10 %>% filter(Dimension %notin%c("time_start", "time_end"))%>%dplyr::mutate(`Los<- / Gain` = dplyr::case_when(loss > 0~ "Loss", loss == 0 ~ "No differences", loss < 0~ "Gain", TRUE ~ NA_character_))  %>% dplyr::mutate(`Loss / Gain` = dplyr::case_when(is.na(`Loss / Gain`) ~ "Gain",value_sum_1 == value_sum_2~"Egal", TRUE ~ `Loss / Gain`))%>% dplyr::ungroup()%>%  dplyr::rename(`Values dataset 1` = "value_sum_1",`Values dataset 2` = "value_sum_2") %>% select(parameter_columns_to_keep) %>%  dplyr::group_by(Dimension, unit,`Loss / Gain`) %>% dplyr::arrange( Dimension, unit,`Loss / Gain`,desc(`Difference in value`)),groups = c("Dimension","unit","Loss / Gain"))
...
if ("Gear"%in% colnames(init) & "Gear"%in% colnames(final) ){
  list_pie_chart <- list("fishingfleet","source_authority","Gear")} else if ("Name_En"%in% colnames(init) & "Name_En"%in% colnames(final) ){list_pie_chart <- list("fishingfleet","source_authority","Name_En")} else {list_pie_chart <- list(setdiff(colnames_to_keep, unlist(list(time_dimension, geographical_dimension, "unit", "value"))))[[1]]}
...
function_pie_chart_df <- function(dimension, ...){
  ...
  if(length(pie_chart) == 2){df = pie_chart$df
