...
if("geographic_identifier" %in% colnames(dataframe_to_filter)) {dataframe_to_filter <- dataframe_to_filter%>% rename("geographic_identifier" := {{geo_dim}})}
...
pie_chart_2_default <- function (dimension, first, second = NULL, topn = 4, titre_1 = "first", 
  titre_2 = "second", title_yes_no = TRUE, dataframe = FALSE, spec_col, rfmo_col) 
{
...
```{r}
pie_charts_multiple <- lapply(list_pie_chart,FUN = function_pie_chart_df, first= anti_join_nomt, topn = 6, title_yes_no = FALSE, titre_1 = "Strata available in NOMT but not in MTNO" )
```
...
```{r}
shapefile.fix <- shapefile.fix %>% rename(geographic_identifier = code)
...
JOINED_DATA <- st_join(world_sf, areas_conversion_factors_numtoweigth_ird) %>% rename(GTA = code.x, IRD = code.y) %>% select(GTA, IRD) %>% mutate(IRD = ifelse(is.na(IRD), 0, IRD))
    
geographic_identifier_to_ird <- function(x){
x <- x %>% full_join(JOINED_DATA, by = c("geographic_identifier" = "GTA")) %>% rename(geographic_identifier_old = geographic_identifier, geographic_identifier = IRD ) %>% distinct() %>% mutate(Year = lubridate::year(time_start), Month = lubridate::month(time_start))
  return(x)
}
...
```{r}
function_pie_chart_df(dimension = "species", first= rdslevel0, topn = 6, title_yes_no = FALSE, titre_1 = "Rds_level0" )
```
...
comparison_converted_factor <- inner_join(conversion_factors_num_to_weight_ird_2021%>% rename(conversion_factor_ird =conversion_factor) %>% mutate_if(is.Date, as.character),converted_level0%>% rename(conversion_factor_gta =conversion_factor), by = c("gear", "Year", "Month", "geographic_identifier", "species", "source_authority")) 
...
espece_parametrage2 <- filtering_function(espece_parametrage %>%rename(species =C_ESP_3L), colnames_to_filter = "species", matchingList = matchingList) %>% select(species, C_OCEA, V_POIDS_MOY)
```
...
```{r}
 ...
 new_table_longer <- inner_join(to_comp %>% ungroup(), max_conversion_factor) %>% mutate(conform = case_when(conversion_factor_rfmo < min_weight ~ "Too low",  conversion_factor_rfmo > max_weight  ~ "Too high", TRUE ~ "INSIDE")) %>% rename(RFMO =conversion_factor_rfmo, IRD =conversion_factor_ird )%>% filter(conform == "INSIDE") %>% pivot_longer(cols = c("RFMO", "IRD"),names_to = "source" ) %>% rename(time_start = time_start.x)
...
cleaned <- rbind(IOTC_DATASETS_CECoastal, IOTC_DATASETS_CELongline) %>% janitor::clean_names()%>% tidyr::pivot_longer(cols = starts_with(c("YFT","BET","SWO", "ALB", "SKJ", "SBF"#, "LOT", "FRZ", "KAW", "COM", "BILL", "TUX", "SKH", "NTAD"
                                                                                                                                           )), names_to = c("species"))%>% separate(species,into = c("species","unit"),sep= "_") %>% filter(!is.na(value))%>% mutate(species = toupper(species), unit = toupper(unit)) %>% rename(fishingfleet = fleet)
```
