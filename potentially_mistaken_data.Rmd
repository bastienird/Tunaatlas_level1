---
title: "Quality analysis of data in Global Tuna Atlas and choices made to treat it"
author:
...
address:
  ...
  - code: IOTC Secretariat
    address: Blend Seychelles, Providence, PO BOX 1011, Victoria, Mahé, Seychelles
output:
...
endfloat: TRUE # Set to TRUE to turn on latex endfloat package to place figures and tables at end of document
...
fancyhdr:
  first:
    headleft: "Global Tuna Atlas Conversion factors analysis"
    headright: "IOTC-2022-WPDCS18"
    headrulewidth: 0pt
  ...
  subsequent:
    ...
    headrulewidth: 1pt
    footrulewidth: 0pt
abstract: "In this paper, we present a brief overview of data provided by tRFMOs presenting potential inconsistencies of declaration. Those inconsistencies are of 3 types: Geographical inconsistency, Conversion factors inconsistency, Plausability inconsistency. They are presented in the order from the less to the more debatable."
keywords: "Geographical declaration, Conversion Factors, Data validation, Data discovery, Data access, Fisheries, CWP, FIRMS, Fishing catch, Fishing effort"
params:
...
Among the declaration by tRFMOs, some data is located on land. The shapefile used for the land profile is available on : fao_url
...
if (isNullList(parameter_filtering)){cat("There are no filter on this data")} else {
  ...
  cat (paste0("The filters used on the data analyzed in this document are:  \n "))
...
#   require(dotenv)
# }
# out <- try(load_dot_env(file = "catch_local.env"))
# if("try-error" %in% class(out)){load_dot_env(file = "catch_local.env")}

# config <- initWorkflow("DB.json") #ok
# entity <- config$metadata$content$entities[[1]]
# action <- entity$data$actions[[1]]
# opts <- action$options
# conn <- config$software$input$dbi

out <- try((con <- config$software$input$dbi), silent = TRUE)
...
image <- tm_shape(st_as_sf(mislocated_continent %>% dplyr::mutate(st_area = as.factor(st_area))))+tm_fill("value")+tm_facets(by=c("st_area","unit"))+tm_borders("grey", lw = 0.1)+tm_shape(continent)+tm_borders()#+tm_text("source_authority")#+tm_shape(continent)+tm_borders() 


image2 <- tm_shape(st_as_sf(mislocated_continent %>% dplyr::mutate(st_area = as.factor(st_area))))+tm_fill("source_authority")+tm_borders("red", lw = 0.1)+tm_facets(by=c("st_area","unit"))+tm_shape(continent)+tm_borders()
...
formals(pie_chart_2_default)$title_yes_no <- FALSE
...
pie_charts_multiple <- lapply(list_pie_chart,FUN = function_pie_chart_df, first= mislocated_continent %>% ungroup() %>% select(geographic_identifier) %>% inner_join(data, by = "geographic_identifier"), topn = 3, title_yes_no = TRUE, titre_1 = "Mislocated data" )
...
The data displayed by tRFMOs is supposed to concern the spatial area of the juridiction of the tRFMO. However, some data is displayed out
...
```{r echo = FALSE, fig.cap='Localisation and contributor to data declared outside source tRFMO zone'}
image
```


Removing data placed on continents, as well as data from an rfmo in an area outside their jurisdiction, the losses at the end of level 0 amount to `r round(perte_en_nombre,2)` % in number and `r round(perte_en_tonne,2)` % in tons. 
...
strange <- studycatchesird::pie_chart_2(fishingfleet,first=absurd_conversion_factor_tons %>% select(colnames(rawdata)),second=rawdata %>% filter(unit%in%c("MTNO","MT")), titre_1 = "Strange conversion factors", titre_2 = "Rawdata")
```
