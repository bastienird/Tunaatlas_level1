...
set_caption(qflextable2(mislocated_continent %>%  group_by(unit) %>% summarise(value = sum(value))), caption = "Value of data reported on continent by unit ",  style = "Table Caption")
```
...
map_unit_knit = function(map){
  ...
  knitr::knit_child(text = c(
    '',
    '',
    '```{r,fig.cap = paste0("Distribution of mislocated data in value")}',
    '',
    'map_for_knit_where',
    '```',
    '',
    '```{r,fig.cap = paste0("Contributor of mislocated data")}',
    '',
    'map_for_knit_who',
    '```',
    '',
...
```{r}
`%notin%` <- Negate(`%in%`)
max_conversion_factor <- read_csv("data/max_conversion_factor.csv") %>% mutate(max_weigt = as.double(max_weigt))
...
```{r results='asis', fig.cap='Repartition of fishingfleet for impossible conversion factors'}
strange
```
