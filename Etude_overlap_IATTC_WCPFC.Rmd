---
title: "Study of the management of data on the overlapp zone between IATTC and WCPFC"
output:
...
This document give a quick analysis of different way to handle the overlapping zone between IATTC and WCPFC. 
...
The current way to handle this area is to keep only data from IATTC as it is the only one to give fishingfleet information.
...
Proposition <- function_overlapped(dataset = Initial_zone_overlapped, con =con, rfmo_to_keep = "IATTC",
...
```{r}
#Paul treatment 
Actual  <- initial_SBF[ which(!(initial_SBF$species %in% "SBF" & initial_SBF$source_authority %in% c("ICCAT","iattc","IATTC","WCPFC"))), ]%>% filter(species == "SBF")
