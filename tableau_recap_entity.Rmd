...
for (sub_list_dir in list_dir){
  ...
  nofish_init <- pull(read.csv(paste0(sub_list_dir_2[1],"/sums.csv"))[2])
    for (i in sub_list_dir_2){
      ...
      loss_tons <- (tons_init - pull(sums[1]))
      loss_percent_lines <- 100*((lines_init - pull(sums[3]))/lines_init)
      loss_percent_no <- 100*((nofish_init - pull(sums[2]))/nofish_init)
      percentage_of_nominal <- (pull(sums[1])*100)/nominal
      data_i <- cbind(step,
                      # Explanation, Fonctions, 
                      Options,
