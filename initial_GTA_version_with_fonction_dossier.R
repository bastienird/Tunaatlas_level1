...
# source("https://raw.githubusercontent.com/BastienIRD/Tunaatlas_level1/main/function_raising_georef_to_nominal_Bastien_old.R")
...
options_raising_georef_to_nominal <- options$raising_georef_to_nominal
...
options_iccat_include_type_of_school <- iccat_ps_include_type_of_school
...
DATA_LEVEL <- unlist(strsplit(entity$identifiers[["id"]], "_level"))[2]
         ...
         if (!is.null(options$mapping_map_code_lists)) if(options$mapping_map_code_lists){
           ...
           # 
           # fonction_dossier("unit_treatment",
           #                  georef_dataset,
           #                  "Removing nomt unit and replacing mtno by mt",
           #                  "map_codelists", c())
         ...
         if (!is.null(options$resolution_filter)){
           ...
           fonction_dossier("filtering_on_resolution_filter",
                            georef_dataset, 
                            "Filtering",
                            "Filter", 
         ...
         if(!is.null(options$aggregate_on_5deg_data_with_resolution_inferior_to_5deg)) if (options$aggregate_on_5deg_data_with_resolution_inferior_to_5deg) {
           # georef_dataset_number <- georef_dataset %>% mutate(geographic_identifier = as.numeric(geographic_identifier)) %>% 
           #   filter(!is.na(geographic_identifier)) %>% mutate(geographic_identifier = as.character(geographic_identifier))
           #sum georef dataset = 125548762
           #sum georef_dataset number = 119641511
           # su mgeoref dataset not number = 5907251
           # df_input_not_number_geographic <- setdiff(georef_dataset, georef_dataset_number)
           ...
           # sum georef_dataset_number = 119641511
           #so the issue is with the not numbered geographic identifier, they are only given by IOTC
           ...
           fonction_dossier("Aggregationon5deg",
                            georef_dataset, 
                            "Aggregate data on 5° resolution quadrants (for 5deg resolution datasets only)",
         ...
         if (options$include_IATTC && options$include_WCPFC && !is.null(options$overlapping_zone_iattc_wcpfc_data_to_keep)) {
           ...
           fonction_dossier("OverlappingIATTCWCPFC",
                            ...
                            "filtering the source authority for identifiers on the zone", c(options_overlapping_zone_iattc_wcpfc_data_to_keep,options_include_IATTC, options_include_WCPFC))
           }
         ...
         # 
         # georef_dataset <- georef_dataset %>% filter(unit != "NOMT")
         # fonction_dossier("unit_harmonization",
         #                  georef_dataset, 
         #                  "Harmonization putting all the MTNO in MT and the NOMT in NO",
         #                  "", c())
         #end swith LEVEL 0
       #-----------------------------------------------------------------------------------------------------------------------------------------------------------
       ...
       #-----------------------------------------------------------------------------------------------------------------------------------------------------------
         ...
         if(!is.null(options$spatial_curation_data_mislocated)){
         ...
         if(!is.null(options$disaggregate_on_5deg_data_with_resolution_superior_to_5deg)){
         if (options$disaggregate_on_5deg_data_with_resolution_superior_to_5deg %in% c("disaggregate","remove")) {
           ...
           fonction_dossier("disagreggate5deg",
                            georef_dataset, 
         ...
         if(!is.null(options$disaggregate_on_1deg_data_with_resolution_superior_to_1deg)){
         if (options$disaggregate_on_1deg_data_with_resolution_superior_to_1deg %in% c("disaggregate","remove")) { 
           ...
           fonction_dossier("disagreggate1deg",
                            georef_dataset, 
