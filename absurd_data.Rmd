...
date: '2022-05-11'
params:
  final: "~/Documents/Tunaatlas_level1/jobs/20220712184945/entities/global_catch_1deg_1m_ps_bb_firms_Bastien_filtering_wcpfc_at_the_end_level0/Markdown/raising"
  plotting: "plot"
...
pie_chart_2 = function(dimension, unit = c("MT","MTNO","Tons","t"), first, second) {
  ...
  provisoire_t <- second %>%  group_by(!!colnames#, unit
    ...
    arrange(desc(value)) %>%   mutate(id = row_number())%>%
    mutate(class = as.factor(ifelse(id<5,!!colnames,"Others"))) %>%
    group_by(class#, unit
    ) %>%  summarise(value = sum(value, na.rm = TRUE))%>%ungroup()%>%
    select( value, class,#unit
    ) %>%#   group_by(unit) %>%
...
con <- DBI::dbConnect(drv , dbname = "tunaatlas_sandbox2", host = "localhost",   port = 5432,
                 user = "tunaatlas_u",
...
all_the_data <- inner_join  %>% ungroup() %>% group_by(geographic_identifier, st_area, unit, source_authority, area_competence) %>% summarise(value= sum(value, na.rm = TRUE))%>% distinct() %>% inner_join(st_make_valid(st_read(con, query = query))%>% filter(!st_is_empty(.)) %>%dplyr::mutate(cat_geo = as.factor(case_when(st_area == 1 ~ "1_deg", st_area == 25 ~ "5_deg", TRUE ~ ">_5_deg"))) , by = c("geographic_identifier"="code", "st_area") ) %>% select(geographic_identifier, st_area, value, geom, unit, source_authority, area_competence)
...
image <- tm_shape(st_as_sf(mislocated_continent %>% mutate(st_area = as.factor(st_area))))+tm_fill("value")+tm_facets(by=c("st_area","unit"))+tm_shape(continent)+tm_borders()#+tm_text("source_authority")#+tm_shape(continent)+tm_borders() 
image


image2 <- tm_shape(st_as_sf(mislocated_continent %>% mutate(st_area = as.factor(st_area))))+tm_fill("source_authority")+tm_facets(by=c("st_area","unit"))+tm_shape(continent)+tm_borders()#+tm_text("source_authority")#+tm_shape(continent)+tm_borders() 
image2
...
```{r}
rawdata<- readRDS("data/rawdata.rds") #%>% filter(source_authority == params$source_authority)
rdslevel0 <- rawdata
...
# original <- pie_chart_2(fishingfleet, mapped_data %>% filter(unit %in% c("Tons","MT","MTNO")))
# original
...
rawdata <- readRDS("data/rawdata.rds")
  data <- rawdata%>% mutate(unit = case_when(unit %in% c("MT","t","MTNO", "Tons")~ "Tons", unit %in% c("no", "NOMT", "Number of fish", "NO")~"Number of fish"))
cl_cwp_gear_level2 <- read_csv("data/cl_cwp_gear_level2.csv", 
    col_types = cols(Identifier = col_skip(), 
        Abbreviation = col_skip(), Name_Fr = col_skip(), 
        Name_Es = col_skip(), Name_Ar = col_skip()))
